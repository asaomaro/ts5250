...
## ラウンド 2（2026-09-08T08:07:54Z）

ラウンド 1 の 16 件はすべて実装と一致（別コンテキストで 1 件ずつ突き合わせ済み）。
**新規の 5 件は、そのうち 4 件がラウンド 1 の修正に由来する**——「同じ不変条件を支える項が
他にも散っていないか」を問い直した結果。

- [must][conv:-] **猶予が明けたのに見ている人が居る場合、持ち主不在のまま普通のセッションに戻す**。
  以後 `claim` する者は現れず、`dispose` は attach 側を `!this.attached` で弾くので、
  その閲覧タブが閉じても誰も畳まない。ブラウザ経路の既定アイドル上限は `"never"` なので
  **枠と装置記述を握ったまま永久に残る**。D10 が閉じた「誰も閉じない」が猶予の解除経路から再び開いていた /
  対応: 修正済（持ち主が居ない点は MCP の放置セッションと同じ状態なので、
  **同じ孤児回収のアイドル上限を付ける**（`orphanSafeIdleTimeoutMs`）。あわせて掃除役の経路も
  タイマー側と同じ `reapHold` に通した。回帰テスト＋変異注入で確認）
- [should][conv:-] ラウンド 1 で `tryResume` の `opened` に足した `settled` ガードが、
  **同ラウンドの must 修正（閉じたあとの `opened` で `close` を送る）を打ち消していた**。
  `closeSession` は `cancel()` で `settled` を立ててから消すので、配送済みの `opened` が
  ガードで弾かれ `close` を送らない。**回帰テストが `closeSession` を通していなかった**ため緑のままだった /
  対応: 修正済（打ち切りの側で `close` を投げてから閉じる。どちらの順序でも正しい。
  テストも本物の入口（`closeSession`）から叩く形に直した）
- [should][conv:-] `refuseIfDisconnected` が `endedByHost` しか見ないため、
  **ラウンド 1 で足した具体的な理由が打鍵 1 回で消える**（3270 の案内・繋ぎ直しを諦めた理由・
  猶予切れの文言が、どれも「サーバーと繋がっていないため送信できません」＝待てば戻る含みに上書きされる）。
  ラウンド 1 が名指しした嘘は、通知の追加ではなくこの合流点に残っていた /
  対応: 修正済（切断中は既に出ている通知を上書きしない）
- [should][conv:-] 繋ぎ直しの門で 3270 だけ通知を足したため、**隣の `attachedOnly` が黙って戻る枝として
  取り残された**（同じ状況で結論だけ逆） /
  対応: 修正済（繋ぎ直さない枝を 1 つの条件にまとめ、どれも案内を出す）
- [should][conv:agents!] `ended` の意味分けを**サーバー側で押さえる検査が 1 件も無い**
  （`dispose` に `ended: true` を足す退行が全緑で通る） /
  対応: 修正済（サーバー側テスト 2 件を追加。変異注入で落ちることを確認）

## ラウンド 3（2026-09-08T08:34:59Z）

ラウンド 2 の 5 件はすべて実装と一致。**新規 5 件のうち 4 件が、またラウンド 2 の修正に由来する**。
2 ラウンド続けて同じ形（散らばった暗黙の連言の、直した項の隣が壊れる）だったので、
**個別に当てるのをやめ、「持ち主の居ないセッションの寿命」を 1 つの規則にまとめた**
（`SessionManager.idleLimitOf`）。

- [should][conv:-] 孤児回収の上限を**付ける経路しか無く、外す経路が無い**。手動の繋ぎ直しで
  持ち主が戻っても 30 分の上限を持ち続け、しかも `lastActivity` は画面を見ているだけでは
  進まないので、**無操作 30 分でホストの対話ジョブごと畳まれる**——この work が消しに来た
  「セッションが黙って消える」が別経路で戻る /
  対応: 修正済（**設定値を書き換えるのをやめ、寿命の規則として重ねる**。持ち主が戻れば
  `claim` で自然と元へ戻る）
- [nit][conv:-] `orphanSafeIdleTimeoutMs` は「個別値が無い＝マネージャ既定に従う」を区別できず、
  `--idle-timeout` を有限に設定したサーバーでは上限が**その設定より長い 30 分へ延びる**
  ——回収を強める意図の 1 行が、設定によっては緩める向きに働く /
  対応: 修正済（`Math.min` で**短いほうが常に勝つ**。回帰テスト 2 件）
- [should][conv:-] **同じ「持ち主不在のセッション」が `dispose` の隣の枝に残っている**
  （持ち主が去るときに他に見ている人が居ると、猶予にも入らず上限も付かない） /
  対応: 修正済（上の規則が `dispose` の経路も自動的に覆う。回帰テストで固定）
- [should][conv:-] `refuseIfDisconnected` の「上書きしない」条件が `notice === undefined` なので、
  **切断より前に出ていた無関係な通知が居座り、切断の理由が一度も出ない**
  （`MSG_NO_RESPONSE` が出ている状態で切れると、打鍵しても最後までそのまま） /
  対応: 修正済（切断の時点で古い通知を捨てる。守るのは「この切断について出した理由」だけ）
- [nit][conv:-] `tryResume` の同一性ガードが `opened` と `error` にしか無く、
  `applyDisplayMessage` へ落ちる枝は**打ち切った試行の口からでも live なセッションを書き換える**
  （`screen` は `connected = true` を立てるので、はしごが回っている最中に「接続中」へ戻る） /
  対応: 修正済（同じガードを合流点に置いた）

**射程を広げすぎた分の差し戻し**: 上の規則を最初「持ち主が居ないセッション全部」に掛けたところ、
既存テスト（`session-idle-timeout`「引数なしのマネージャは、いくら時間が経っても掃除しない」）が
落ちた。MCP / HLLAPI が開いたセッションは**開くときに自分で寿命を決めている**ので、
対象を**「持ち主が居て、去った」もの**（`hadHolder`）に絞り直した。

## ラウンド 4（2026-09-08T09:29:42Z・判定）

**追加のラウンドは回さない。** ラウンド 3 の差し戻しで上限（`maxSendBacks` 3）に達し、
プロトコルどおり**まっさらなコンテキストへ原因究明だけを委譲**した（`decisions.md`「デバッグ D1」）。
診断は「個別の欠陥ではなく規則の置き場所」で、次のアクションは `stop_for_human`。
**利用者の判断で、いまの内容を PR として出し、判定の畳み込みは別 work とする**。

ラウンド 3 の指摘 5 件はすべて反映済み（うち 1 件は射程を広げすぎて既存テストを落としたため、
対象を「持ち主が居て、去った」ものに絞り直した）。原因究明が挙げた「隣の項」4 件のうち、
**事実として確認できた 2 件**を処置した:

- [nit][conv:-] `sweepIdle` のプリンター枝だけ新しい寿命規則（`idleLimitOf`）を通らない /
  対応: **対応不要**——プリンターは猶予の対象外で、転送断ではその場で閉じるため
  「持ち主が去って宙に浮く」状態が作れない。main と同じ扱いのままである旨をコメントに明記した
- [should][conv:agents!] `refuseIfDisconnected` の JSDoc が「**送信の入口すべてがここを通る**」と
  書いているが、VT とプリンターの送信口（`vt-input` / `printer-*`）は通らない——**過大な主張** /
  対応: 修正済（「5250 表示セッションの送信の入口は」に直し、VT / プリンターが通らない理由
  （繋ぎ直しの対象外で、切断の見せ方を自前のペインが持つ）を書いた）

残る 2 件（`EmulatorPane` の 2 層通知・`tryResume` の `error` 枝の非対称）は、いずれも
**到達経路が確認できていない推測**なので、畳み込みの work へ送る。
