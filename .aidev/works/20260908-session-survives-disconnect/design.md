...
interface SessionManagerOptions {
  /**
   * 転送断で保持する猶予（ms）。既定 60_000。**0 で無効**＝従来どおり即閉じ。
   *
...
interface SessionState {
  ...
  /**
   * **転送が繋がらないまま試行が尽きた**（手動の繋ぎ直しを出す）。
   * 猶予切れ（`SESSION_NOT_FOUND`）では立てない——押しても同じ理由で失敗するため。
   */
...
- **猶予中のセッションは `maxSessions`（既定 8。F6）の枠を占め続ける**。常駐プリンターのように
  ...
  占有は最大 60 秒に限られる（D5）。ホスト側の装置記述も同じ間だけ掴んだままになるが、
...
- **31 秒が猶予 60 秒の内側に収まる根拠**（起点の違い）:
  - **`close` が飛ぶ切れ方**（サーバー再起動・明示的な切断）: 両者が同時に気づく。
    サーバーは即 `holdForReconnect`、クライアントは即再試行 → 31 秒は 60 秒の内側。
  - **半開き**（`close` が来ない）: クライアントは `ping` が 90 秒来なくて気づき、
    サーバーも `pong` が 90 秒来なくて気づく。**同じ最後の往復から数えるので、
    気づく時刻はほぼ同時**。したがってこの場合も、クライアントの 31 秒は
    サーバーが猶予に入れた直後から始まり、60 秒の内側に収まる。
    先に試行が届いた場合は**そもそも猶予に入る前**で、セッションは生きているので成功する。
