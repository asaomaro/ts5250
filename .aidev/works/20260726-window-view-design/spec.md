...
「ウィンドウそのもの」と「背景」は目的が別（前面を目立たせる／後面を引っ込める）なので、
**独立した設定**にする。ただし設定行が増えるのは避けたいので、**1 つの畳んだ行にまとめ、
開いたときにセクションで区切る**。

| 設定 | 値 |
|---|---|
| `windowFrame`（ウィンドウ） | 無効 / 影 / 浮き出し / 枠強調 |
| `windowBackdrop`（背景） | 無効 / スモーク / すりガラス / ぼやけ |

- 旧 `windowView` の保存値は 2 項目へ読み替える（`smokeShadow` → 影＋スモーク 等）。
- キー設定の順送りは**項目ごと**に効く（`view:windowFrame` / `view:windowBackdrop`）。

## D7. `VIEW_ITEMS` に `group` を足す

同じ `group` を持つ項目を 1 行にまとめる。`groupLabel` を先頭の項目に書く。
設定モデル自体は平坦なまま（項目ごとに独立）なので、順送り・保存・移行はそのまま効く。
...
|---|---|
| スモーク | 半透明の黒を重ねる |
| すりガラス | `backdrop-filter: blur() saturate()` ＋ うっすら地色。下に何かあることは分かるが読めない |
| ぼやけ | `backdrop-filter: blur()` のみ（色を足さない） |

`backdrop-filter` は重ねた要素の**背後**をぼかすので、文字そのものには触れない（桁も不変）。
