...
- [must][conv:-] D33の`draft`（`ref`）をそのまま`postMessage`していた——前ラウンドの修正に由来。同じ不変条件（「host へ送る値は構造化複製できる」）を
  支える箇所は`postToHost`1か所に集まっているので、そこで写すようにし、テストの差し替えも複製を通す形にした。
- 入口の HTML の`no-cache`は Web 版の`index.html`にも効く（更新後に古い画面が出る問題は Web 版にもある）。
