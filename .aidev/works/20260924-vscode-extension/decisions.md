...
## D27: `S`の継ぎ目の隙間・傾き・上半分の幅を直す

- **背景**: 利用者の指摘（SVGを拡大した画像つき）——(1)Sの中央に極わずかな縦の隙間がある (2)Sが僅かに傾いて見える
  (3)下半分より上半分の幅が大きく見える。
- **(1)の原因**: SVGでは`S`を2本の線（上の弧・下の弧）で描き、中央で端を突き合わせていた。ブラウザは各線の縁をぼかして描くので、
  継ぎ目に細い隙間が出る。PNG・ICOは点の内外判定で塗るので出ない（SVGだけの症状）。
  → `S`を**1本の閉じた輪郭（塗り）**で描く（`sOutline()`）。上の弧の外縁の下端と下の弧の内縁の上端は同じ点なので、
  輪郭は弧4本と切り口の線分2本で閉じる。拡大して、変更前にあった継ぎ目が消えたことを確かめた。
- **(2)の原因**: 円弧の端は半径の向きに斜めに切れる（40°と220°）。上下の切り口が平行な斜め線になり、S全体が傾いて見えた。
  → 端を**縦に切る**（上は x = 中心+4、下は x = 中心−4.5。内側の円より内で切り、切り口を縦1本にする）。
  ラスタは弧の判定に「この角度の範囲では x がこれより右（左）なら外」を足し、SVGは輪郭に縦の線分を入れる。
- **(3)の原因**: 同じ半径の2円を180°回した形。→ 上の半径を小さく、下を大きくした（6.75 / 7.75。字高 34 = 2r1 + 2r2 + W は保つ）。
  書体でも上を小さくするのが普通の補正。
- **比べた案**（Chromiumで並べて目視）: 半径の比だけ変える／端の角度だけ変える／クリップで縦に切る（中央付近の弧まで欠けた）。
  上の3点をすべて直せたのは「上を小さく＋端を縦に切る＋1本の輪郭」だけだった。

## D28: `TS`を自作の図形から書体（Noto Sans CJK JP Medium）の字形に替える

- **背景**: 利用者の指摘「Sの文字のバランスがおかしい」と見本の画像（普通のサンセリフの`TS`）。D27で継ぎ目・傾き・上下の幅は直したが、
  円弧だけで組んだ`S`は背（中央の斜めの線）が無く、書体の`S`の釣り合いにはならない。~~D26/D27の「矩形と円弧で組む」~~は破棄する。
- **決定**: 書体の字形をそのまま使う。環境にある書体（Noto Sans CJK JP Regular / Medium / Bold・DejaVu Sans / Bold）で`TS`を
  見本と並べ、字形と太さが一番近い **Noto Sans CJK JP Medium**（ラテン文字はSource Sans系）を選んだ。
  - fontkitで`font.layout("TS")`（カーニング込み）の輪郭を取り、キャップハイトを揃えたまま2文字の幅を46に収め
    （そのままだと53で地の角丸に寄りすぎる）、字の縦の中心 y=28・外形の中心 x=32（下線の中心）に置いた。
    輪郭は`gen-icons.mjs`の`GLYPHS`に座標（小数3桁）で埋め込み、生成は引き続き外部依存なしで動く。
  - ラスタ（PNG/ICO）は、輪郭を折れ線に崩して行ごとに交点を求め、非ゼロ規則で塗る実装に替えた（円弧・切り口の判定は削除）。
    SVGは輪郭をそのまま1本の`path`で出すので、継ぎ目は原理的に出ない。
  - Noto Sans CJKはSIL OFL 1.1。字形の輪郭を図案に使うのはライセンスの範囲（書体ファイルそのものは配らない）。出所は`GLYPHS`のコメントに書いた。
- **検証**: 見本・SVG・PNG（1024）・VSCode（128）・ICO（32/16/24）を並べて目視。SVGとPNGは同じ形、16pxでも`TS`と読める。
  `test/app-icons.test.ts` 3 passed・`--check` 一致。

## D29: ~~下線のカーソルをやめ、`S`をブロックカーソル（■）で反転表示する~~（D30で破棄）

- **背景**: 利用者の要望「エミュレータはカーソルと■で表現するので、アイコンも画像のイメージでカーソルを含んだ表現にして」と図案
  （黒地にピンクの`T`、緑の■の上に黒で抜いた`S`。下線なし）。
- **破棄（D30）**: 図案の読み取りが違っていた。利用者の指定で「`S`は緑・黒地のまま」「`T`の背景を白に」「下線を戻す」に直した。以下は当時の記録
- **決定**: 図案どおりにした。`T`は`--t-pink`（#ff00ff）、■は`--t-green`、■の上の`S`は地の色（`--crt`）で抜く。下線は廃止
  （■がカーソルを表すので二重になる）。~~D28の配置（字の縦の中心 y=28・下線の中心に合わせる）~~は下線が無くなったので組み直した:
  キャップハイト26、`T`・間3・■（`S`の外形＋左右3）の全体を x=32 に中心合わせ、■は y 14〜50（字の縦の中心 y=32）。
  ■の角は丸めない（端末のセル）。
- **`T`の色は図案の読み取り**（届いた画像が小さい）。緑の`T`と並べた比較も作った。違っていれば`T_COLOR`の1行で変わる。
- **検証**: `test/app-icons.test.ts`に色（ピンク）と描く順序（地→T→■→Sを地の色）を足して3 passed。SVG・PNG（1024）・VSCode（128）・
  ICO（32/16/24、白地）を並べて目視。

## D30: `T`にブロックカーソル（■）を重ねる（D29を破棄）

- **背景**: 利用者の要望「エミュレータはカーソルと■で表現するので、アイコンもカーソルを含んだ表現にして」と図案。最初は図案を
  「緑の■の上に黒で抜いた`S`・ピンクの`T`・下線なし」と読み取って作ったが、利用者の指定で直した——
  「`S`は元のままグリーンで背景も黒のまま」「`T`の文字色は合っているので背景を白に」「グリーンの下罫線を追加」。
- **決定**: 白（`--t-white`）の■の上にピンク（`--t-pink`）の`T`、黒地に緑の`S`、緑の半透明の下線（従来どおり）。
  ~~D28の配置~~は■に合わせて組み直した: キャップハイト26、■（`T`の外形＋左右3）・間3・`S`の全体を x=32 に中心合わせ、
  ■は y 10〜46（字の縦の中心 y=28。下線 y=51 の上に5空く）。■の角は丸めない（端末のセル）。
- **検証**: `test/app-icons.test.ts`に4色（地・白・ピンク・緑）と描く順序（地→■→T→S→下線）の検査を足して3 passed。
  SVG・PNG（1024）・VSCode（128）・ICO（32/16/24、白地）を並べて目視。

## D31: 下線を`S`と同じ緑の角の帯にし、■と接するよう配置を組み直す

- **背景**: 利用者の指定——「下罫線の色も`S`の文字色に揃えて」「横幅を伸ばし、端の丸めを角に」「下罫線と`T`の白背景を接触させる」
  「白背景を延ばすのではなく、レイアウト調整で接触させる」。
- **決定**:
  - 下線は`--t-green`の不透明（以前は55%の半透明）、角は丸めない。幅は■の左端から`S`の右端まで（46.744。以前は36）。
  - ■の高さ（36）は変えず、■＋下線（41）をアイコンの縦の中心 y=32 に置き直した: ■ y 11.5〜47.5、下線 y 47.5〜52.5。
    字は■の縦の中心（y=29.5）に合わせて1.5下げた（横の配置は変えない）。
- **検証**: `test/app-icons.test.ts`に「■の下端＝下線の上端」「下線に`rx`/`opacity`が無い」「下線は■の左端から」を足して4 passed。
  SVG・PNG（1024）・VSCode（128）・ICO（32/16/24、白地）を並べて目視。

## D32: 種別ごとに拡張子を分け（`.ts5250emu`等）、空のファイルから画面だけで設定できるようにする

- **背景**: 利用者の指摘「接続前の設定画面で種類を変更できない。そのため空の設定ファイルを用意して画面から設定できない」。
  案は「設定画面で種類を選ぶ」「種類ごとに拡張子を分ける」の2つを示し、利用者が**拡張子を分ける方を選んだ**（「既存の.ts5250とは両立不要」）。
- **決定**:
  - 拡張子は `.ts5250emu`（端末）/ `.ts5250prt`（プリンター）/ `.ts5250spl`（スプール）/ `.ts5250sql`（SQL）/ `.ts5250ifs`（IFS）。
    表は`EMBED_APP_EXTENSIONS`（`protocol.ts`/`embed-protocol.ts`。手で同期を保つ複製）の1か所。`package.json`の`customEditors`/`languages`は
    静的なので表を参照できず、`test/package-json.test.ts`が突き合わせる。**`.ts5250`は受け付けない**（両立不要の指定）。
  - 種別は拡張子から決め、**ファイルには`app`を書かない**（読み込み時に入れ、書き出し時に外す。書かれていても無視）。
  - **空のファイル（空白だけ）は「何も設定していない」として開く**——`fileInvalid`にしない。これで空ファイルを作って開けば画面から設定できる。
  - 画面のファイル名表示・名前（タイトル）は種別の拡張子を使う。`.gitignore`に5つの拡張子を足した（暗号文を含むので秘密と同じ扱い。AGENTS.md「セキュリティ」）。
- **ついでに直したこと**: `vscode-extension`の`typecheck`（`tsc -b tsconfig.test.json`）がmainで落ちていた（D23のテストで`panel.dispose()`
  ——モックの型では呼べない）。`panel.fireDispose()`に直した。

## D33: 設定の自動保存をやめ、「保存」ボタンで書く。「接続」「開く」は未保存の変更を保存してから繋ぐ

- **背景**: 利用者の指摘「設定画面で1文字入力する度に保存が掛かるので、保存ボタンを配置して、クリックするまで保存しないように。
  接続・開くボタンクリック時は保存してから接続するように」。~~D23の自動保存（400msの間引き）~~・~~D24の「自動保存しました」表示~~を破棄する。
- **決定**:
  - 入力は保存しない。「保存」ボタン（変更があるときだけ押せる）で`save`を送る。拡張ホスト側はそのまま（`applyEdit`＋`document.save()`）。
  - 「接続」「開く」は、未保存の変更があれば`save`を先に送ってから`connect`を送る（拡張ホストはメッセージを順に処理する。D23）。
    変更が無ければ`connect`だけ。
  - 状態表示: 変更あり「未保存の変更があります」→「保存しています…」→「保存しました」。応答待ちの間に次の入力があれば「未保存」のまま。
  - `SettingsForm`は打ちかけの不正な値（ポート）の間`change`を`undefined`で出し、その間は「保存」「接続」とも押せない
    （以前は`change`を出さないだけで、呼び出し側は古い値のまま保存・接続できてしまう形だった）。
  - タブを閉じる直前の自動保存（`pagehide`）もやめた——保存ボタンがある以上、押さずに閉じた変更は捨てる。
- **検証**: mutation 9件（接続前の保存を消す・保存の条件を外す・入力ごとに保存・不正値でも接続可・表示の拡張子・空ファイルの受理・
  `app`を書き出す・`app`の優先順位・`package.json`の拡張子を1つ消す）——初回は2件が生き残り（保存の条件／`app`の優先順位）、
  テストを足して（保存済みの後の「接続」が保存を送り直さない）または当てるテストを直して（schemaのテスト）すべてkilled。

## D34: 「保存」「接続」が送れず「保存しています…」のまま止まる不具合を直す／入口の HTML をキャッシュさせない

- **背景**: 利用者の報告「保存ボタンクリック後、保存はされていても『保存しています…』の表示から変わらない」。
- **再現（実際の VSCode 1.139 で）**: `@vscode/test-electron`で VSCode を起動し（WSLg の画面）、拡張を読み込み、`--remote-debugging-port`から
  embed.html の中を操作して確かめた。入力→「保存」で「保存しています…」のまま、ファイルは書き換わらない。画面のコンソールに
  `DataCloneError: Failed to execute 'postMessage' on 'Window': #<Object> could not be cloned.`。
- **原因1（DataCloneError）**: D33で入力中の値を`ref`（`draft`）に持つようにした。`ref`に入れた値は Vue が Proxy で包むので、それを
  `postMessage`に渡すと構造化複製ができずに投げる——「保存」も、保存してから繋ぐ「接続」「開く」も送れない。jsdom のテストは`postMessage`を
  `vi.fn()`に差し替えていたため複製を通らず、見逃した。
  → `postToHost`で**常に素の JSON に写してから送る**（呼び出し側を問わず効く）。テストの`postMessage`の差し替えも`structuredClone`を通す形にした。
- **原因2（古い画面が出続ける）**: 直したビルドを入れても、画面が古い JS（`embed-C1M5_bgD.js`）を読み続けた。`embed.html`（と`index.html`）に
  `Cache-Control`が無く、ブラウザが最終更新日時から鮮度を見積もってキャッシュしていた——**拡張を入れ直しても古い画面が出る**
  （D21の「emulator が接続ボタンだけ」の報告もこれだった可能性がある。未確認）。
  → 入口の HTML に`Cache-Control: no-cache`を付けた（`serveStatic`の`onFound`は応答を作った後に呼ばれて効かないので、前段のミドルウェアで付ける）。
  `/assets/*`はハッシュ付きの名前なので付けない。
- **原因3（同じ処理の中の入力と押下）**: 入力と「接続」の押下が同じ処理の中で起きると、フォームの`change`（描画前にまとめて届く）が押下より後になり、
  保存を飛ばして古い設定で繋いだ（CDP で入力と押下を1回の評価で行ったときに再現）。→ `SettingsForm`の監視を`flush: "sync"`にした。
- **検証（実際の VSCode）**: 修正後、入力→「保存」で約1.9秒後に「保存しました」・ファイルに反映。入力→「開く」で保存してから画面が開く
  （ファイルに入力値）。コンソールに DataCloneError なし。単体: 修正を戻すと`postToHost`のテストと「接続は未保存を先に保存」のテストが落ち、
  `flush`を戻すと同期のテストが落ちる。
