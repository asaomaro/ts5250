...
同じ表に CCSID 273 の列と 5035 の列が同居しうる（検証表 `MYLIB.CSVUPJP` が実例）。
...
|---|---|
| CSV を渡して行が追記され、SQL で読み返して一致 | 実機。`MYLIB.TESTPF`（CHAR(5)×4・CCSID 273） |
| 日本語を含む行が書け、読み返して一致 | 実機。検証表 `MYLIB.CSVUPJP`（CCSID 5035/930 列）。**CL 経由は使えない**（F9） |
