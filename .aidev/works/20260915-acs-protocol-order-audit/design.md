...
- 対象外（変更しない）: ~~`default:` 節の設計そのもの（ACS 式への全面的な作り替え。
  `research.md`「実現性/リスク」）~~ → **`20260921-wtd-control-bytes` で破棄**（ACS のコアで制御バイト入りの WTD を測り、後続のオーダーがすべて処理され、
  制御バイトは 1 桁の表示データになることを確かめた。`default:` 節は無くなった）、`ORDER.UNKNOWN_1C` の実装・位置づけの
