...
| 11 | 6S0 の~~数字桁を満杯（5 桁）~~ **6 桁中 5 桁**→ Enter | **エラー** |
...
- ~~場合 11（符号付き数値の数字桁を満杯）: 当 PJ は満杯で次の欄へ自動送りするので、欄を出たことになり送れてしまう。~~
  → 誤り（確かめずに書いた）。当 PJ は符号桁に留まり自動送りしない＝ACS と同じく 0020（`20260921-field-exit-required-types` で確かめた）。
  ACS は RB/RZ/符号付き数値の欄を満杯でも自動送りしない（`isFieldExitRequired`）——台帳の「RB/RZ 欄のフィールド終了」の範囲。
- Help で出た「機能キーは使用できません」の出所（ACS かホストか）。

> **訂正（`20260921-field-exit-required-types` の独立点検の後）**: 場合 11 の「数字桁を満杯（5 桁）」は読み違い。6S0 はワイヤ上で
> 7 桁（数字 6 桁＋符号桁）なので、5 桁では満杯ではない。**6 桁打てば Enter は送れた**（同 work research F4・場合 A）。
> 結果（エラー）そのものは正しい——満杯でないので 0020。
