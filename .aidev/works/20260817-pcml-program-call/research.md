...
`scripts/research-pcml.mjs`。実機で RPG IV を作り、コンパイル時に吐かせた。

```
CRTBNDRPG PGM(TESTLIB/PCMLTST) SRCFILE(TESTLIB/QRPGLESRC) SRCMBR(PCMLTST)
...
`scripts/research-pcml-layout.mjs`。A で作った `PCMLTST` を、
