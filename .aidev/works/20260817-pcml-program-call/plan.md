...
| 5 | 実機 | `scripts/verify-pcml-osaka.mjs` | **SR-OSAKA で往復** |
...
## 5. 実機（`scripts/verify-pcml-osaka.mjs`）

`research` で作った `ASAOLIB/PCMLTST` を、**今度は PCML 経由で**呼ぶ。
`research-pcml-layout-osaka.mjs` と**同じ値・同じ判定**にして、
...
| 実機の後片付け | `PCMLTST` は `ASAOLIB` に残す（他の試験片と同じ扱い） |
