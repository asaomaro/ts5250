...
`scripts/research-pcml.mjs`。RPG IV を作り、コンパイル時に PCML を吐かせた。
タグは **819**、1,086 バイト。全文は `research.md` A に採ってある。

### 2. 宣言どおりのバイト並びか — **8 PASS / 0 FAIL**

`scripts/research-pcml-layout.mjs`。**PCML の宣言だけを頼りに生バイトで組んで**呼んだ。
...
`scripts/verify-pcml.mjs`。**生バイトを 1 つも組まない**。記述は IFS から読む。
...
### 2. 名前だけで呼ぶ
  PASS 呼び先は TESTLIB/PCMLTST（path から解いた）
  PASS 引数は 7 本
  PASS 呼び出しが成功
### 3. 名前で読む
...
`scripts/verify-browser-pcml.mjs`。画面の操作だけで一通り。
...
OK 呼び出しが成功する — 成功（戻り 0） 呼び先 TESTLIB/PCMLTST
