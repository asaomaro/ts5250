...
**実機**: 交渉は同じところまで進むが、**画面を 1 バイトも送らない**（生 43 B ＝ 交渉のみ）。
...
**この過程で backlog の宿題も 1 つ片付いた**——`tn3270-ibmi.md` に「実機が装置名を拒む理由が
...
⚠ **実機の QMAXSIGN は 3**。実機検証でサインオンを失敗させる試行は 3 回までに抑える。
