...
- [x] T8: `mcp-tools.ts` に `open_printer_session`/`wait_spool`/`list_spools`/`get_spool`（close は流用）。
      戻り値は等幅テキスト。（依存: T6）
- [x] T9: `profiles.ts` にセッション種別（任意項目）。（依存: T6）

## web-ui
- [x] T10: `stores/sessions.ts` を kind 判別 union 化（既存 display はそのまま、printer 状態
      reports[]/selectedReportId を追加）。（依存: T5）
- [x] T11: `ConnectView.vue` にセッション種別（display/printer）選択を追加し `open.kind` を送る。（依存: T10）
- [x] T12: `PrinterPane.vue`（左=受信スプール一覧／右=等幅帳票ビュー／保存ツールバー: テキスト＋印刷→PDF）。
      タブ内容の kind 振り分けで printer→PrinterPane。単体: 論理ページ→行描画のロジック。（依存: T10,T13）
- [x] T13: `ws-client.ts`/`session-controller.ts` で `report`/`printer-status` を処理し store へ反映。（依存: T7）
