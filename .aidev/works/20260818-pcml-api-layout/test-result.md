...
`scripts/verify-pcml-api-osaka.mjs`。`jtopen` 同梱の `qsyrusri.pcml` を**1 文字も変えずに**使った。
...
### 4. 返った値
  ...
  userProfile   = "ASAO"    status = "*ENABLED"    badSignonAttempts = "0"
...
`scripts/verify-browser-pcml-api-osaka.mjs`。IBM の記述を**貼り付けて**操作した。
...
OK bytesReturned=83 / userProfile=ASAO / status=*ENABLED
