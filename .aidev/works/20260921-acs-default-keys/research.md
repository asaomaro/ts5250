...
- F4（原典）: `processEndField` は `Field5250.getEndPosition` を使う——欄の終わりから非空白を探し、見つけた桁が最後の桁ならそこ、そうでなければ次の桁、
  ~~無ければ欄の先頭~~ → **探す下限はカーソルの行の先頭**（欄の先頭より前なら欄の先頭）で、無ければその下限（節目の独立点検で原典と違うと
  指摘され、`getEndPosition(n, n2)` の `n` を読み直して確認）。継続欄は `FFT5250.getEndPositionOfContField`（最後の区切りから遡る）。
  欄の外なら `nextNonByPassInputFieldPos` で次の入力欄へ。
