...
## 〔追記・訂正〕IBM i が 3270 を喋る — 調査の穴だった

**この research の結論は不完全だった。** 「日本語 DBCS ホストはローカル構築できない」までは正しいが、
そこから「日本語ホストは要らない（s3270 との照合で足りる）」と結論したのが早計だった。
**IBM i の telnet サーバーが 3270 端末を受け入れることを検証していなかった。**

手がかりは目の前にあった——F6 で参照した `x3270` のパッケージ説明にこう書いてある:

> It can also connect to an IBM AS/400 as a 5250 terminal **using the AS/400's 3270 emulation**.

読んで報告までしておきながら、「3270 ＝ メインフレームの話」と決めつけて追わなかった。

### 実測（deliver 後に確認）

| ホスト | 3270 での応答 |
|---|---|
| **pub400（IBM i 7.5）** | 英語のサインオン画面。**入力欄あり**（`Your user name:` / `Password`）・非表示欄あり |
| **社内機（IBM i 7.3・日本語）** | **日本語のサインオン画面**（`サイン・オン` / `ユーザー` / `パスワード`） |

両機とも `IBM-3279-2-E` を受理し `connected-3270` に到達する。

### これが意味すること

1. **実ホストでの入力往復が検証できる**——TK4- に無かった「入力を受け付ける画面」が IBM i にはある。
2. **実機の日本語 DBCS が検証できる**——自作データストリームだけに頼らなくてよい。
3. 逆に、**TK4- だけで検証していたことがスコープ判断を誤らせた**（下記）。

### IBM i 接続に要った 3 点（TK4- では不要だった）

| 要件 | TK4- | IBM i |
|---|---|---|
| コマンドコード | **EBCDIC 系**（F5/F1/F3…） | **SNA 系**（05/01/**11**…） |
| WSF Query | 撃ってこない | **撃つ。応答しないと画面を出さない** |
| 画面の届き方 | 直接 | **Outbound 3270DS に包まれる** |
| DBCS の申告 | 不要 | **日本語機は CharacterSets の DBCS 記述子が要る** |

**spec で「WSF は記録して読み飛ばす」とスコープ外に置いた 2 点が、ちょうど IBM i 接続の必須要件だった。**
検証環境を 1 つに絞ると、その環境が要求しないものがスコープから落ちる——という形の失敗。
