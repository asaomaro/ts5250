...
### 実行したもの
- `cd packages/scs && npx vitest run test/scs.test.ts` — 54 passed / 0 failed（全角空白が行末に来るケースの `cols` 確認を 1 件足した）

### 受け入れ基準の再確認
- 変更なし。追加のテストは既存の受け入れ基準（全角空白は下の字を消さない）の裏付けを、桁数の面から補強するもの。

### mutation
`scratchpad/mut-scsn7.py`（全角空白で `maxCol` を進めない）— 検出（KILLED）。
