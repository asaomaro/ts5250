...
- [x] **`READ SCREEN TO PRINT`(0x66/0x68/0x6A/0x6C) / `READ IMMEDIATE`(0x72/0x83) は
  ...
  - ~~⚠ `0x72`/`0x83` は本来**応答が要る**。当方は応答していない（警告で明示）~~ → 応答するようにした（`20260822-read-immediate`・PR #347、`20260822-datastream-all`・PR #348。2026-09-27 の照合で取り消し）
...
- [x] **未知のコマンドに対する負応答** — ~~**要らないと分かった**~~ → ACS と同じ条件で返す（`20260921-negative-responses`）
  ...
  - 原典 2 つは負応答を返すが、**返さなくても止まらない**。~~形式を確かめられないものを推測で
    送るより、黙って捨てて**警告で気づけるようにする**方を採る（従来どおり）~~ → 形式は ACS の原典で分かり
    （`DS5250.tokenizeData`）、**入力コマンドでは返さないとホストが待ち続ける**ことを実測した（WSF D9/72 のフラグ 0x80。
    ACS では CPFA304 で戻る）。ACS と同じ条件で否定応答を返すようにした（`20260921-negative-responses`）。
    未知のコマンドは ACS も~~否定応答を返さず~~ 1 バイト読み飛ばして続ける（~~残りを捨てる~~ も ACS に合わせた）。
    **パラメータが 2 バイト以上あれば**、読み飛ばした先が ESC でないので ACS も 0x10050121 を返す（節目 9 の独立点検の指摘）
  - ~~⚠ 残る危険は「**未知のコマンドの後ろに READ がある**」形。パラメータ長が分からない以上
    捨てるしかない。ただし**読み取り系はこれで全部実装した**ので、その口はかなり狭まった~~ → 捨てずに ACS と同じく 1 バイト読み飛ばす
    （後ろの READ も、未知のコマンドのパラメータが 1 バイト以下なら読める）
