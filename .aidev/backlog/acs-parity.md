...
- [ ] **J（DBCS 専用・`only`）欄の空きの桁へ離れて打つと、途中に半角空白が残る**（優先度 中・深さ ◐。`20260921-g-field-sosi` の調査で見つかった）。J の欄に `あ` が入っていて、離れた空き桁へ `い` を打つと、
  編集の値が `あ   い`（半角空白 3）になる（`padDbcs` の詰め物が半角空白）。**core は J の欄の半角空白を弾く**（`validateFieldContent` の `isDbcsOnly`＝「全角しか入力できない」）ので、送信で止まる。
  ACS の J の空きは全角空白（0x4040）で、打った字はその桁に入る。G は詰め物を全角空白にして直した。J は空きが NUL のホストの欄と、SO/SI 込みの桁数の扱いがあり、詰め物を全角空白にすると
  「触っただけで値が変わる」を避ける詰め物の落とし方（`trimPad`）と合わせる必要がある。着手時に ACS の J で NUL 埋めの欄への打鍵を測る。
