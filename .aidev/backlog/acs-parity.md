...
- [x] **Ctrl+Home（罫線）・Ctrl+F11（カーソルの形）を既定のキーにする**（下の「キー編集の細部」の R11 (h) の残り）。**完了（`20260921-default-keys-rule-cursor`・PR #410）**:
  ACS の既定の割り当て（`AcsMapFunctions.MAP_5250`）の `C36 = [rule]`・`C122 = [altcsr]`。当 PJ に機能（`view:ruleLine`・`view:cursorShape`）はあり既定のキーだけが無かったので、
  `keybindings.ts` の版 5 で足した（版 4 以前の保存値には追加分だけを足し、使用中のキーは奪わない）。単体 3 件、mutation 3 通りすべて検出。GUI 層のキー割り当てなので実機では測っていない（原典の表の読み）。
- [x] **語頭ジャンプ（`[tabword]`・`[backtabword]`）を ACS の規則にし、Alt+←/→ を割り当てる**（下の「キー編集の細部」の R11 (h) の残り）。**完了（`20260921-word-tab-acs`・PR #410）**:
  実機の ACS のコア（社内機・930。メニューの画面。`scripts/acs-probe/tabword.txt`）で、**全角は 1 字ごとに止まる**（36・38・40…）・SI の直後の半角も語頭・**画面の端で巻き戻る**（右下から前へ 1,2・左上から後ろへ 24,32）と分かった。
  当 PJ の `nextWordStart`（`packages/web-ui/src/composables/useCursor.ts`）は非空白の連なりを語・各行の 1 桁目を常に語頭・端で停止だった。ACS の `is1stCharacter`・`get1stCharPosition` の規則
