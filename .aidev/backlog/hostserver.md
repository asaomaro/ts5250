...
- [x] **7.3 と 7.5 で挙動が違う箇所を洗う**（上の項目から分割）
  ...
  - → `20260822-host-release-diff`（PR #349）で両機を同じ手順で叩いて並べた
    （`scripts/compare-hosts-pub400.mjs`。再実行できる）。
...
- [x] IFS の書き込みで `dataCcsid` を明示し、新規作成ファイルのタグを中身と合わせる
    ...
    - **既定タグは機械ごとに違う**。~~850~~ → 実機は **1041** / PUB400 は **850**。
      ジョブ・システムの既定に依る
    ...
    - バイナリ（base64）とアップロード経路は**触っていない**（文字コードの概念が無い）。
      65535 でタグ付けするかは別の判断
    - 再現: `scripts/research-ifs-dataccsid.mjs`
...
- [x] SQL 結果表の行を仮想化する（表示範囲だけ描く）
  ...
  - 基準線: `scripts/research-sql-table-render.mjs` /
    確認: `scripts/verify-sql-table-virtualize.mjs`（16/16）
...
- [x] `host_call_program` を正しいパラメータ列で成功させる検証
  - 実機確認は `MCH0802`（パラメータ数不一致）までで、**呼び出し経路が通ることしか確かめていない**
    ...
    - 再現: `scripts/research-call-program.mjs`（副作用なし）
...
- [x] **64KB を超える LOB の分割受信と、LOB の打ち切り（`too-large`）を実機で確かめる**
  ...
  - 再現: `scripts/research-lob-multi-segment.mjs`（往復を生で覗く）/
    確認: `scripts/verify-lob-multi-segment.mjs`（14/14）
...
- [x] **自動出力付きプリンターセッションを常駐化する**
  ...
  - 残る論点: プリンターは**受信したスプールの出力（PDF・印刷）まで常駐で行う**ので、
    ...
    - 実機の再現: `scripts/verify-printer-residency.mjs`
...
- [x] **常駐プリンターを画面から扱えるようにする**（上の続き）
  ...
  - 残っていたのは「**未読の帳票が読める**」だけで、しかも**配線は途中まで出来ていた**
    ...
    - 実機の再現: `scripts/verify-printer-report-history.mjs`（13/13）。
