...
- [x] MCP の `host_sql` から非クエリ文を実行させるか決める
    ...
    → **実行させると決めた**（`20260822-hostserver-mcp-and-lob`・PR #352）。
      ⚠ **SELECT 専用に縛っても安全にはなっていなかった**——`host_command` から
      `RUNSQL 'DELETE …'` を撃てば同じことができる。止まっていたのは
      「構造化された影響行数が返ること」だけだった
    → 代わりに **`allowWrite: true` を明示したときだけ**通す。**意図を毎回述べさせる**もので、
      「SELECT のつもりで打った文が更新だった」を防ぐ。**接続する前に断る**ので、
      断るつもりの呼び出しでホストへ出て行かない。応答は `kind: "execute"` と
      `updateCount` / `hasRowCount` / `warning`
...
- [x] **`host_sql` の取得量を制御できるようにする**
  ...
  - **ブロッキング係数も残りに合わせる**——既定 100 のままだと上限 1 でも 100 行ぶん届く
    ...
    （1 往復の応答を膨らませない）
- [x] 画面のページング経路も上限つき取得に寄せるか決める
    → 今回は**触っていない**（`pageSize` 指定は結果セットを保持して続きを読む別の要求）。
      ただし「1 ページだけ見て閉じる」使い方が多いなら、保持せず打ち切る方が接続を掴まない。
      使われ方を見てから決める
    → **寄せないと決めた**（`20260822-hostserver-mcp-and-lob`・PR #352）。
      **心配していた事象はすでに起きない**——読み切った（`hasMore: false`）ページは
      その場で結果セットを閉じて接続をプールへ返している（`host-sql.ts`）。
      つまり「1 ページだけ見て閉じる」でも接続は掴まない
    → 寄せると**「続きを読む」が成立しなくなる**（結果セットを保持しないので）。
      掴む問題だけが理由なら、経路は分けたままでよい。
      振る舞いを固定するテストを足した（`host-sql-limit.test.ts`）
...
- [x] **しきい値をアプリから指定できるようにするか決める**（上の続き）
  ...
  - ~~**効果が最も大きいのは PUB400（1 往復 4〜7 秒）**だが、そちらでは未実測~~
    → **pub400 で実測した**（`scripts/research-lob-threshold.mjs` を両機で走らせられる形にした）。
      LOB セル 6 個の SELECT で **12 往復 / 5,014ms → 4 往復 / 1,306ms（3.8 倍）**。
      SR-OSAKA（LAN）では受信バイトが 132,757B → 5,078B
  - → **公開すると決めた**（`20260822-hostserver-mcp-and-lob`・PR #352）。
    `/api/host/sql` と MCP `host_sql` の `lobThreshold`（上限 15MB）
  - ⚠ **プールの鍵に混ぜないと黙って効かない。** しきい値は接続時の属性
    （`setServerAttributes`）であとから変えられないので、鍵に入れないと
    「しきい値 0 で張った接続」を「65536 で開いたつもり」の要求に貸してしまう。
    応答は正しく見えるのに往復だけ増えたまま、という気づけない形で出る
