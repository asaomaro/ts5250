...
- [x] **しきい値をアプリから指定できるようにするか決める**（上の続き）
  ...
  - ~~**効果が最も大きいのは PUB400（1 往復 4〜7 秒）**だが、そちらでは未実測~~
    → **pub400 で実測した**（`scripts/research-lob-threshold.mjs` を両機で走らせられる形にした）。
      ...
      実機（LAN）では受信バイトが 132,757B → 5,078B
...
- [x] 常駐プリンターを**長時間（数時間〜数日）保てるか**を測る（上から分割）
  ...
  - 併せて検討: `SessionManager` の 30 分アイドル掃除は常駐サービスには効かせない。
    上限 8（表示・プリンター合算）の枠の扱いも決める
  → `20260822-printer-idle-keepalive`（PR #354）。**保てていなかった。測って直した。**

  ### 測ったこと（実機・7.3）

  - **上限**: 55 件投入 → **保持 50 / 累計 55 / PDF 55**（落ちた帳票もファイルには残る）
  - **アイドル**: 15 分後にスプールを流すと**届かない**（90 秒待って 0 件）。
    ⚠ **そのときの `entry.state` は `listening` のまま**（20 分でも変わらず）
    ——**こちら側からは障害が全く見えない**

  ### 原因の切り分け

  症状が同じでも原因は 2 つある（A: こちらのソケットの無言死 / B: ホスト側ライターの終了）。
  **どちらか決めずに直さない**ため、アイドル明けに両側を同時に見た
  （`diag-printer-idle-cause.mjs`）。**ライターは動いたまま**（`状態=STR`・滞留 0）で **B は否定**。

  ### 直したあと

  | | 修正前 | 修正後 |
  |---|---|---|
  | アイドル後の配信 | 15 分で**届かない** | **50 分でも 1 秒で届く** |
  | ホスト側ライター | `STR` | `STR`（同じ） |

  ⚠ **張り直しは一度も発火していない**（`state` が `reconnecting` にならない）。
  **効いたのはキープアライブ**——原因 A が確定した。

  ### 直した中身

  - **TCP キープアライブ**（主役）。`tn5250` / `vt` / `tn3270` / `ddm-transport` に
    **入っていなかった**。`hostserver` 側（45 分保つ待ち行列監視が使う）には入っていた
    ——**入っていない方だけが落ちていた**。
    ⚠ 効き方は「落とされない」が主で、「気づく」は OS 任せで数分かかる
  - **落ちたときの張り直し**（保険）。状態機械は `listening → reconnecting → listening` を
    定義しているのに**プリンターだけ `error` へ直行**していた。web-ui は
    `reconnecting` を描く用意ができていた——**サーバーが出していなかっただけ**
  - 張り直しを入れて見つかった穴 2 つ:
    `close()` が張り直しを降ろさず**捨てたエントリが接続を張り続ける** /
    `startRescue()` が前のタイマーを消さず**救出が二重に走る**

  ### 併せて検討する項目は既に決着していた

  30 分掃除は `sweepIdle` が `resident` を飛ばし、常駐は表示の枠を食わず独自の上限を持つ。
  どちらも `printer-residency.test.ts` が固定済み。**決めることは残っていなかった。**

  ### 他の経路も測った

  | 経路 | 実機 | pub400 |
  |---|---|---|
  | 5250 表示 | **30 分**越え | **30 分**越え |
  | 3270 | **30 分**越え | **30 分**越え |
  | VT | VT を出せない | **7 回中 6 回死亡** |
  | プリンター | 上のとおり | 試験不可（`CPF3464` 権限。セッションは張れる） |

  ⚠ **VT だけは解けていない。** 同時刻の並走（同じホスト・同じ 23 番）で
  **VT 2 本が死に 5250 は生きる**ので経路のばらつきではなく **VT 固有**。
  **TCP キープアライブも telnet `IAC NOP` も効かなかった**（NOP は検証して外れたので撤去）。
  ホスト側の対話ジョブは 30 分間生きたままで `close` も届かない。**原因は特定できていない。**

  ### 限界

  **「数日」は未検証**（測ったのは 50 分まで）。キープアライブは**経路の機器次第**なので、
  別のネットワークでは別の閾値になる。**VT は直っていない**
