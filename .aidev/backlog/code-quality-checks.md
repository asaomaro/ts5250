...
- [ ] fieldEdit.paste() が typeChar を既定値（need=1 / reserved=0）のまま呼ぶ（優先度 低・深さ ◎＝調査済み）。20260920-insert-mode-overflow で打鍵・DBCS・IME・貼り付けの 4 経路に取り置きを通したが、この関数だけ本番の呼び出し元が無く（テストからのみ）対象外にした（decisions D3/D9）。将来使われると符号桁・DBCS 桁を守らない経路が復活する。使うなら roomArgsOf 相当を渡すか、使わないなら削除する（出典: .aidev/works/20260920-insert-mode-overflow/decisions.md）
- [ ] onCompositionEnd の SBCS 経路が base ではなく e を渡している（優先度 中・深さ ◐）。ScreenGrid.vue の IME 確定は dbcs ? dbcsType(base, ...) : typeChar(e, ...) と書かれており、選択を消した直後に挿入扱いへ切り替える base（composeReplacedSelection）が SBCS 側に効いていない。20260920-insert-mode-overflow で気づいたが、直すと選択置換時の IME 確定の挙動が変わるため同 work の範囲外にした（decisions D10）。まず ACS 実機で挙動を測ってから直すこと（出典: .aidev/works/20260920-insert-mode-overflow/decisions.md）
- [ ] verify-browser-*.mjs のうち 14 本が旧世代の connections.json を読む（優先度 中・深さ ◎＝調査済み）。現行は profiles.local.json（ServerConfigStore.fromFile）で 10 本が移行済み。.env.verify が指すシステムは profiles.local.json 側にあるため、旧世代の 14 本はこの環境で起動できない（verify-browser-sign.mjs で実際に落ちた。20260920-insert-mode-overflow decisions D7）。棚卸しして移行するか、動かないものは削除する（出典: .aidev/works/20260920-insert-mode-overflow/decisions.md）
