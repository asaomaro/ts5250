...
| `research-edtmsk.mjs` | 調査用。`DTMPGM` の受信を **core を通さず独立にパース**して SF オーダーを並べ、**`EDTMSK` が欄を分解するか**を見る。⚠ **2026-08-25 訂正: 分解する**（下記の注意）。当時「分解しない」と結論したのは、マスクの `&` を数字の桁に置いていて `CRTDSPF` に無視されていたため。接続先は**環境変数を優先**する（`connections.json` に実機が載っていないことがある。2026-08-26 に修理）。 |
...
| `verify-browser-edtmsk-edit.mjs` | 回帰 E2E（実ブラウザ＋実機・**63 項目**）。区切りの**色が入力欄と同じ**／**下線が `border-bottom` で同じ高さ**／末尾からの **Backspace ×3 が区間をまたいで詰まる**／先頭の **Delete が後続区間から桁を引き寄せる**／`2026/08/25` を**区切り込みでペーストしても桁がずれない**／**Tab が区間ごとに止まらない**（並び全体で 1 つの欄）。加えて**日付・時刻ピッカー**（⑥⑦）: `D8U` で選んだ日付が**ホストへ届いて返る**／`TMW` は**空欄だと区切りが画面に出ない**ので日付/時刻のタブが出て、値が入ると時刻に確定する。⚠ 8 桁打つと自動送りで欄を出るので、Backspace の前に最終区間へ置き直すこと（さもないと別の欄を消す）。⚠ ピッカーは**既定 OFF** なので `addInitScript` で `as400.view.settings` を入れてから開く。⚠ `TMW` の編集ワードは先頭に `0` が無く**時の先頭ゼロがホスト側で抑制される**（`09` を送ると ` 9:30:15` で返る）ので、往復の比較には 10 以上の時を使う。⑧ は**キーボードだけ**の経路（`Alt+↓` で開くとフォーカスがピッカーへ移り、矢印と `Enter` で全区間を決め、`Esc` で欄へ戻ってそのまま送れる）——**種別が確定した後の `TMW`** を使うことでタブを経由せず堅牢に測れる。**`Tab` が部品の中で巡回すること**もここで見る（jsdom は合成 `keydown` で実際のタブ移動を起こさないので、**本物のタブ移動は実ブラウザでしか測れない**）。 |
...
## AID の応答待ちと施錠中の逃げ道（実機 / 資産不要）

**画面（web-ui）の応答待ちには期限を設けない**（`.aidev/backlog/aid-response-timeout.md`）。
原典（tn5250j の `tnvt.java` / lib5250 の `session.c`）にも実機の OIA にも「時間で諦めて施錠を解く」
動作は無く、`X SYSTEM` は点いたまま待つのが正常。旧実装は既定 30 秒で打ち切り、**まだ走っている
のに「応答がありませんでした」と出し、`state` を `ready` に戻して施錠を偽って**いた。
代わりに抜ける口を **Attn / SysReq**（システム要求メニュー「2. 前の要求の終了」）に置いた。

| スクリプト | 何を見るか |
|---|---|
| `verify-aid-no-timeout.mjs` | `DLYJOB DLY(60)` を `timeoutMs: "never"` で送り、**35 秒たっても待ちを打ち切らない・施錠も解けない**こと（旧実装なら 30 秒で諦めている）。その最中に **Attn が `KEYBOARD_LOCKED` を投げずに通る**こと。**SysReq「2」で走っている要求を実際に切れ**、待っていた AID が解決すること。テスト用の資産は要らない（コマンド行だけ）。 |
