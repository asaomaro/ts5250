...
| `build-dttest.mjs` | 実機の `TESTLIB` に `DTMDSPF`/`DTMPGM` を作成（冪等）。**`EDTMSK`（編集マスク）つき入力欄**を 8 通り（`&` / 空白 / `&&/&&/&&` のマスク・`EDTCDE(Y)` 単独・`EDTWRD` の時刻・SSN・素の数値）並べる。**マスクの綴りを推測せず 1 件ずつ単独コンパイル**して通る書き方を実機に教えてもらう。加えて**8 桁の日付欄 `9999/99/99`** を 5 通り（`EDTWRD('    /  /  ')` 単独・ゼロ抑制止め `'0   /  /  '`・それに `EDTMSK` の 2 綴り・`EDTCDE(Y)`）を行 18〜22 に並べる。DDS の定数は**英数字のみ**（日本語だと `INSERT` が長さ超過する。実測 155 > 153）。`DT_SKIP_PROBE=1` で単独コンパイルを省略できる。 |
...
- **8 桁の日付欄（`9999/99/99`）でも同じ**（2026-08-25・実機・`DTMDSPF` の D8W/D8Z/D8M/D8B/D8Y）。
  `EDTWRD('    /  /  ')` を `8 0` の用途 B に付けると**長さ 10 の 1 欄**（`ffw=4300`＝num-only・
  属性バイト `0x24`＝緑＋下線）で来る。`EDTMSK` を足しても **FCW は 1 つも付かない**
  ——**ワイヤに「どの桁が区切りか」を示す材料は無い**ので、端末が `/` を飛ばすには
  **欄の現在値（`2026/08/25`）を読むしかない**。
  - 入力側は**ホストが編集文字を落として数字だけ拾う**: 空欄に `20260825` を左詰めで送ると
    `2026/08/25` が返る。**空欄なら「8 桁打つだけ」で正しく入る。**
  - **ただし欄に既に `2026/08/25` が出ている状態で 8 桁を打つと壊れる。** 区切りを上書きして
    `2026123125`（10 桁）になり、しかも**数値専用欄は `/` を打鍵で入れ直せない**
    （`fieldValidate.ts` の numeric は `[0-9.,+-\s]` のみ）。
  - そのときホストは **WTD で属性バイトを `0x24` → `0x25`（緑＋**反転**＋下線）に書き換え**、
    WEC（`0x21`）で「小数部分の使用法が正しくないか，あるいは入力した数字が多すぎる。」を出し、
    Read MDT（`0x52`）で入力を待つ。**欄が反転表示になるのはホストの指示**であって
    こちらの描画ではない（生バイトで確認）。`EDTCDE(Y)` は `8 0` だと `nn/nn/nnnn`
    （`20/26/0825`）で、`9999/99/99` にはならない。
