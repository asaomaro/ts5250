...
> 📌 **延ばす量は `calc(0.125em + 0.5px)`**（0.125em ＝ 行送り 1.25 の半行送り、+0.5px は端数の丸め代）。
> 実機の HTML で振った実測:
>
> | 延ばす量 | 継ぎ目の濁り | 塗った高さ / 上限 |
> |---|---|---|
> | 0.125em ちょうど | 10 画素 | 18px / 18.75px ✅ |
> | **0.125em + 0.5px** | **0** | 19px / 18.75px ✅ |
> | 0.125em + 1px | 0 | 20px / 18.75px ❌（拡大率 2 ではみ出し） |
> | 0.2em | 0 | 21px / 18.75px ❌ はみ出し |
>
> 地色が透ける画素はどれも 0。**理論値ちょうどだと上下の影が境界でぴたり出会う**ため、
> 端数の丸めで被覆の足りない画素が同系色の細い線として残る（実機の画面でも 5 か所中 4 か所）。
> 丸め代は**1 画素未満**に留める——超えると隣の行の内容領域へ食い込み、**画面2 が機械的に捕まえる**:
> 0.2em に戻すと塗った高さが 36px（上限 32.5px）になり、上の行の内容領域に隣の帯の色が
> 936 画素入る＝ 4 項目が落ちる（実測で確認済み）。
