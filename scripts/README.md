...
> 0.2em は**セル 1 つぶんを超えて塗る**ので、**理論値の 0.125em を採る**——残るのは地色ではなく
> 同系色の 1px で、地色の横線とは別物。**この「超えて塗る」は画面2 が機械的に捕まえる**:
> 0.2em に戻すと塗った高さが 36px（上限 32.5px）になり、上の行の内容領域に帯の色が 936 画素入る
> ＝ 4 項目が落ちる（実測で確認済み）。
