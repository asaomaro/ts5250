...
| スクリプト | 内容 |
|---|---|
| `verify-3270-keys-osaka.mjs` | 実機で F3 / F4 / F12 / F13 とページ送りが効くことを確かめる |
...
### 3270 の装置名（IBM i には通らない）

⚠ **IBM i は 3270 の接続で装置名を受け付けない**（実測。`verify-3270-devname-osaka.mjs`）:

| 渡し方 | 結果 |
|---|---|
| 端末タイプに `@名前` | **交渉が 15 秒で時間切れ** |
| NEW-ENVIRON の `DEVNAME` | 交渉は通るが **Query Reply の直後に CLOSE**。画面 0 文字 |
| TN3270E の `CONNECT` | **IBM i が TN3270E を提示しない**ので使えない |

名前を 4 通り（`AS3270A` / `TST3270X` / `QPADEV0099` / `D3270TST`）試して全て同じで、
`DEVNAME` の 1 行を止めるだけで通った。ホスト側も **3270 の装置記述が 0 件**で、
表示装置は全て `DSPVRT`（IBM i が自動採番する仮想表示装置）。

`DEVNAME` は RFC 4777 の**5250 向け**の仕組み。3270 の装置名は TN3270E の `CONNECT`
（RFC 2355）で渡すのが筋なので、IBM i がそれを提示しない以上この接続では道が無い。

**いまの実装**: IBM i にはどちらも送らない（指定しても壊れない）。
使えないときは警告を出す。`@名前` は NEW-ENVIRON を使わないホスト（TK4- 等）には従来どおり。
