...
SR-OSAKA で来ない理由は QSYSOPR のメッセージで確定している——VT で繋いだ時刻に

```
CPF1194  サブシステム QBASE がワークステーション QPADEV000C をオフに構成変更した
```

仮想装置は**作られている**。そこでジョブが起きずサブシステムがオフにしている。ホスト側の構成の
話で、クライアントからは動かせない。`VtSession` は**この状況を切断理由に書き添える**
（「繋がらない」だけでは手掛かりにならないため）。
