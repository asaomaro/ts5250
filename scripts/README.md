...
| `verify-services-pane.mjs` | 回帰 E2E（実ブラウザ＋実機・14 項目）。**サービス一覧のペイン**: 定義が一覧に出る／**一度も開いていない定義は「未起動」**／サービス ☐ は「対話型」と分かる／**PDF 保存先のパスは画面に出ない**（有無だけ）／**タブを開いても接続が増えない**（`sessions.size === 0` で確認）／**一覧から起動できる**（起動応答 `I902`・常駐として立つ）／停止 → 再開／ルートが `editable` を返す。設定は一時ファイルで、実機の `profiles.json` には触らない。 |
