// 実ブラウザ（web-ui）で **PCML ペイン**を SR-OSAKA に対して一通り操作する E2E。
//
// 確かめるのは「**手詰めが要らないこと**」——
...
//       scripts/research-pcml-osaka.mjs で ASAOLIB/PCMLTST と .pcml を作ってあること。
// 実行: node --env-file=.env scripts/verify-browser-pcml-osaka.mjs
...
const PORT = 3491;
const PCML_PATH = process.env.PCML_PATH ?? "/home/ASAO/pcmltst.pcml";
...
try {
  ...
  check(
    "path から呼び先が読める",
    (await page.locator(".section .note").first().textContent())?.includes("ASAOLIB"),
    "画面に /QSYS.LIB/ASAOLIB.LIB/PCMLTST.PGM が出る"
  );
  ...
  check("呼び先が返る", verdict.includes("ASAOLIB/PCMLTST"), "");
