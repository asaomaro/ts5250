// 実機の TESTLIB に **SQL 画面の検証用オブジェクト**を作って残す。
//
...
//   SQLDEMOV    ビュー
//   SQLDEMOP    手続き（IN ＋ OUT）           → `CALL TESTLIB.SQLDEMOP(1, 1.50, ?)`
//   SQLDEMORS   手続き（結果セット 1 個）      → `CALL TESTLIB.SQLDEMORS()`
//   SQLDEMORS2  手続き（結果セット 2 個）      → 「2 個のうち 1 個目」の表示を見る
//   SQLDEMOF    関数（スカラー）              → `SELECT TESTLIB.SQLDEMOF(ID) FROM …`
 ...
 * `profiles.local.json` に実機が入っている、という並びもある。
 */
function target() {
...
const DDL = [
   ...
   * 文名を分ける・実行し直す…と 10 通り試して実機で確認した）。
   *
