...
// （`scripts/diag-vt-osaka.mjs` / `scripts/README.md` の VT 節）。
//
...
//
// 実行:
//   node --env-file=.env scripts/setup-vt-subsystem-osaka.mjs status    # 今の姿を見る
//   node --env-file=.env scripts/setup-vt-subsystem-osaka.mjs apply     # 作って始める
//   node --env-file=.env scripts/setup-vt-subsystem-osaka.mjs rollback  # 元に戻す
//
...
const SBSD_LIB = process.env.VT_SBSD_LIB ?? "ASAOLIB";
