...
// **走らせるのは `diag-read-immediate-osaka.mjs`**（5250 セッションから CALL してトレースを見る）。
//
// 実行: node --env-file=.env scripts/build-rdimm-osaka.mjs
...
const LIB = process.env.RDIMM_LIB ?? "ASAOLIB";
...
const run = async (c, { allowFail = false } = {}) => {
  try {
    ...
    if (r.success === false) throw new Error(`rc=${r.returnCode}`);
...
if (check === undefined || check.success === false) {
  ...
  out(`\n完了（${LIB}/${PGM} を確認）。走らせるのは scripts/diag-read-immediate-osaka.mjs`);
}
await cmd.close();
