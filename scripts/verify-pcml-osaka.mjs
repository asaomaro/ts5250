...
//
// `research-pcml-layout-osaka.mjs` は同じプログラムを**生バイトの手詰め**で呼んだ。
// ここは同じ値・同じ判定を**名前だけ**で行う——手詰めが消えたことを示すのが目的。
//
// 記述は**実機の IFS から読む**（コンパイラが `PGMINFO(*PCML)` で吐いたもの）。
//
// 実行: node --env-file=.env scripts/verify-pcml-osaka.mjs
// 前提: scripts/research-pcml-osaka.mjs で ASAOLIB/PCMLTST と .pcml を作ってあること。
import {
  CommandConnection,
  IfsConnection,
  parsePcml,
...
try {
  ...
  check(call.library === "ASAOLIB", `呼び先は ${call.library}/${call.program}（path から解いた）`);
