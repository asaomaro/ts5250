...
- 忠実なフィールド編集（**5250 上書き既定 / Insert トグル / 5250 流バックスペース**）、**フィールド型別の入力検証**
  ...
  ペーストでは送らない＝カーソルが動かないため）。**Field Exit が必須の欄**（FER・右寄せ `CHECK(RZ)`/`CHECK(RB)`・符号付き数値）は
  ACS と同じく満杯でも送らず欄に留まる（Field Exit か Tab で出る）
- **画面全体の自由カーソル**（入力欄外も含む任意セルにカーソル配置。矢印で 1 セル移動）
- **タブ / 矢印**でのフィールド移動（上下は桁を保ち真下へ）、**Ctrl+矢印 / Alt+←→でカーソル頭出し**
  （全角は 1 字ごと・画面の端で次/前行へ巻き戻る。ACS の規則）、**マウスホイールでページ送り**（PageUp/Down）
...
- **ホストに切られたら自動で繋ぎ直す**（ACS と同じ。`SIGNOFF ENDCNN(*YES)`・無操作の切断・回線断のあと、
  1 回目は即座に、以後 20 秒おきに試して新しいサインオン画面へ戻る）。自分から切ったとき・サインオンが拒否されたときは繋ぎ直さない。
  ブラウザから開いた端末だけが対象で、MCP の自動操作の接続は繋ぎ直さない（自動操作が予約している間に切られたときも繋ぎ直さずに終える）
- **画面設定（⚙ 画面）**: SO/SI を `{ }`（非表示 / 薄目 / 濃目。ホストのデータにある本物の `{ }` と
  ...
  **ACS の表示設定**も同じメニューにある——カーソル（形状 ブロック / 下線・明滅・マウスポインター 標準 / 十字線）、
  **罫線**（ON / OFF・カーソルに従う・十字線 / 水平 / 垂直）、**桁区切り**（`DSPATR(CS)` をドット / 線 / オフ。
  ドットは入力欄の下線に桁ごとの点が並ぶ）。
  各項目はキーバインドで順送りにもできる（罫線の ON / OFF を ACS の「罫線」キーのように割り当てられる）
...
- **日本語（IGC 属性）の帳票も書き出し経路でそのまま受けられます**（DBCS の CCSID では ACS と同じく
  `IBM-5553-B01` を申告し、装置が 5553 として作られるため。日本語機で確認）。~~IGC（DBCS）属性の帳票は
  書き出し経路を通りません~~（以前の申告では装置が 3812 にされ `CPA3303` で止まっていた）。詰まったスプールの
  **取得経路（pull）での救出**も残してあります
...
2. **操作**:
   ...
   - **Ctrl+矢印 / Alt+←→** でカーソル頭出し（←→=前後の語頭。全角は 1 字ごとが頭で、画面の端では次/前行へ巻き戻る。↑↓=同じ列の非空白セル）
   - **Enter / F1–F24**（Shift+F… で F13–F24）、**PageUp/Down**（マウスホイールでも）
   - **先打ち**: 応答待ち（画面が施錠されている間）に打ったキーは**捨てずに溜め、応答が来たら打った順に入ります**
     （ACS と同じ）。溜めた Enter や F キーも応答の後に送られ、その先に打ったキーは次の応答の後に続きます。
     **Reset（左 Ctrl）・Attn・SysReq で溜めを捨てます**。Insert は溜めずにその場で切り替わります
   ...
   - **ローカル編集キー**（ホストへは送らず端末内で完結します。「⌨ キー」から好きなキーへ割り当て直せます）
     - **Field Exit**（既定 Ctrl+Enter）: カーソル以降を欄末尾まで消し、**フィールドの指定どおり右寄せ**して
       ...
       **Tab や Enter で欄を出たときは右寄せしません**——実機の 5250 と同じで、右寄せしたいときに押すキーです。
       右寄せ・符号付き数値の欄に打ったまま**欄を出ずに実行キー（Enter・F キー・PageUp/Down）を押すと、
       ACS と同じく送らずに操作員エラー**になります（左詰めのまま登録されるのを防ぐため）。
       Field Exit で出るか、Tab 等で一度欄を出てから押してください
     - **Erase EOF**（**既定のキーはありません**。キー設定で割り当てます）: カーソルから欄末尾まで消去。欄からは出ず、右寄せもしません
       （ACS の既定にも Erase EOF のキーは無く、Ctrl+Delete は下の Delete Word です）
     - **Delete Word**（既定 Ctrl+Delete。ACS の `[deleteword]`）: カーソルの語を消して後ろを詰めます。語の頭なら語と続く空白、語の途中ならカーソルから語の終わりまで
       （記号は語の一部で、区切りは空白だけ）。空白の上と全角は 1 字。継続欄（日付欄など）は並び全体を 1 つの欄として数えます。欄からは出ず、カーソルも動きません。
       操作員エラーの間も受け付けます（エラーを抜けてから消します。Delete などの編集キーは拒否）
     - **Erase Input**（既定 Alt+End。**Ctrl+Backspace は何もしません**——ACS に割り当てが無く、以前の既定だった Erase Input が誤って全欄を消すため外しました）:
       **変更した入力欄だけ**をクリアして、画面のホーム位置へ（ACS と同じ。ホストが値を入れてきただけの欄は消しません）
     - **Field− / Field+**（テンキーの − / ＋。既定 Ctrl+- / Ctrl++ も）: 符号付き数値欄の符号を確定して次の欄へ
     - **Dup**（既定 Shift+Insert・Ctrl+D）: カーソルから欄末尾までを複写文字で埋め、次の欄へ（DDS の `DUP` の欄だけ）
     - **End**: 欄の中で、入力した文字の直後（欄が埋まっていれば最後の桁）へ移ります（ACS と同じ）
     - **Newline**（Shift+Enter）: ホストへは送らず、次の行の先頭の入力欄へ移ります（ACS の New Line）
     - **Reset**（**左 Ctrl を単独で押して離す**。ACS の既定）: 操作員エラーを解き、挿入モードを上書きに戻します。
       Ctrl+C のように他のキーと組み合わせたときは Reset になりません
   - **必須の欄**（DDS の `CHECK(ME)` / `CHECK(MF)` / 自己点検）は ACS と同じ時機で止めます——
     ME（必須入力）は**画面を変更したときだけ**、実行キー（Enter・F キー・PageUp/Down）で見ます。
     **CA キー（DDS の `CAnn`。F3 など）では見ない**ので、必須欄が空でも画面から抜けられます。
     MF（全桁入力）と自己点検は、**カーソルのある欄**を実行キーのときに、**欄を出るとき**（Tab・矢印・クリック）にも見ます
   - **操作員エラー**（型違反・符号桁への入力・保護領域への入力・挿入の余地なし・欄を出ずに実行キー・必須の欄 等）が出ると、
     ACS と同じく**文字・Backspace・Delete を受け付けなくなります**（エラーに気づかず打ち続けても
     欄が書き換わりません）。**矢印・Tab・Home などのカーソル移動、Enter 等の AID キー、Reset、
     クリック**で解けます。エラーになった時点で挿入モードは上書きに戻ります（ACS と同じ）
...
3. **見た目**: 設定は広さで 2 つに分かれます——**外観**（アプリ全体。端末と Web アプリの配色・
   ...
   窓枠・画面フォント・カーソル・罫線・桁区切り …）。端末の配色は 2 種類——**5250 端末 クラシック**
   （既定。ダークは ACS の標準色＝黒地に純色、下線も文字色のまま）と **5250 端末 ソフト**
   （以前の既定の、淡いフォスファ調）。通常モード（明るい地）はどちらも同じペーパー調です。**表示はセッションごとに個別指定でき**、指定した項目には ● が付きます
   （プリンター／スプールのタブにも出ます）。
...
| `retry_printer_output` / `cancel_printer_output` | 自動出力（PDF 保存・自動印刷）に失敗して**ホストへの印刷完了の応答を止めている帳票**を再試行／取消（ACS のプリンター・エラーと同じ。止めている帳票は `wait_spool` / `list_spools` の `held` に出る） |
...
- **セッション設定側**（種別を問わない）: `deviceName`、`deviceNameRetry: true`（装置名が使用中なら
  末尾の数字を繰り上げて**同じ接続の中で答え直す**。既定 false ＝「その名前で繋ぎたい」意図をすり替えない。
  ~~繋ぎ直して再試行~~ → 8902（使用中）のときだけ。誤ったパスワードで何度も繋いでサインオンの失敗回数を使い切らない）、
  ...
  - **5250（表示・プリンター）の装置名は ACS と同じく大文字にし、置換記号を展開して送ります**（ACS の「ワークステーション ID」と
    同じ書き方が使えます。3270・VT は書いたとおりに送り、記号の展開も `deviceNameRetry` の繰り上げもしません）:
    `%`（表示は `S`・プリンターは `P`）、`*`（セッション名。当 PJ では `A`）、`=`（**使用中なら 0→1→…→9→A→…→Z と
    同じ接続の中で進める番号**。2 つ以上なら乱数の位置から）、`&COMPN`（機械名）・`&USERN`（利用者名）、`+`（長すぎる名前の左を残す）。
    `&COMPN` / `&USERN` は**このサーバーの機械名**と、認証が有効なら開いた人の利用者名（無効ならサーバーの OS の利用者名）です
    （ブラウザの機械名はサーバーからは取れません）。**`&` を含む名前では記号以外の文字は落ちます**（ACS と同じ。`T%*&USERN` の `T` は送られない）。
    記号の無い名前が使用中なら ACS と同じく繋がりません（`deviceNameRetry` で繰り上げられます）
  - **`sessionType: "display"` のときだけ**効くもの: `terminal`（`"5250"` 既定 / `"3270"` / `"vt"` →
    ...
    **`associatedPrinter` とは同時に書けません。常駐（サービス ✅）のプリンターは止めも閉じもしません**。
    MCP・HLLAPI から開く表示には効きません（ACS の画面の層の機能）。~~待ち時間が切れても表示は開き、プリンターの設定の装置名で関連付けます~~ 待ち時間が切れても（プリンターを開始できなくても）表示は開きますが、**関連付けなしで開き**、その理由を通知に出します。指したプリンターは必ず起こします（「自動で待ち受け開始」が☐でも）。同じプリンターを使う表示を同時に開いても接続は 1 本です。関連付けているプリンターはアイドルの掃除で消しません。参照されているプリンターの設定は削除・種別変更できません（先に関連付けを外します））、
...
（tn5250 は DBCS 自体が未実装で先例にならない）、**ACS と同じ `IBM-5555-C01` を画面サイズによらず使います**。
画面サイズは Query Reply で申告し、24x80 を選べば `*DS4` を持つ `STRSEU` も 80 桁で来ます（実機で確認）。

| | 24x80 | 27x132 |
|---|---|---|
| SBCS | `IBM-3179-2` | `IBM-3477-FC` |
| DBCS | `IBM-5555-C01` | `IBM-5555-C01` |

~~DBCS 24x80 は `IBM-5555-G02`~~（以前は PUB400 の総当たりで 24x80 に `G02` を採っていました。当時の Query Reply は
常に 27x132 可と申告しており、`C01` だと `STRSEU` が 132 桁で来たためです。`20260921-dbcs-terminal-type`）。
DBCS でカラーなのは `C01` と `G02` です（`IBM-5555-B01` / `G01` はモノクロで、掴むと青・桃・黄が落ちて 4 色になります。
...
- **申告は ACS と同じ組です**（`20260921-printer-acs-declaration`）。端末タイプは DBCS の CCSID なら `IBM-5553-B01`、
  SBCS かホスト変換（`transformTo`）なら `IBM-3812-1`。NEW-ENVIRON では装置名のほか、メッセージ待ち行列
  （`QSYSOPR` / `*LIBL`）・`IBMFORMFEED`・`IBMTRANSFORM` と、DBCS なら `IBMIGCFEAT=2424J0`、SBCS なら
  `IBMFONT=11` / `IBMBUFFERSIZE=768` を送ります（`KBDTYPE` / `CODEPAGE` / `CHARSET` は送りません）。
  接続に成功すると起動応答 `I902`（Session successfully started）が返り、ホスト側に仮想プリンターデバイスが作られます。
  - **既にある装置名で繋ぐと、ホストが装置の型を申告に合わせて作り変えます**（3812 の装置に DBCS で繋ぐと 5553 になる。
    ACS でも同じ）。
  - DBCS の申告では装置の文字セットがシステムの既定になるので、**英語機（PUB400 など）では日本語は置換されます**。
  - ACS と同じく、ホストの取り消し（CLEAR）には応答し、受けかけの帳票を閉じます。
...
- ~~**IGC（DBCS）属性の帳票は、この書き出し経路を通れません。** 仮想プリンター装置は SBCS の
  `TYPE(3812) MODEL(1)` として作られるため~~ → **DBCS の CCSID なら通ります**（装置が 5553 として作られる）。
  **SBCS の CCSID で開いたセッション**に IGC 属性の帳票を回すと、装置が 3812 なのでホストが `CPA3303`
  （… ユニコード/IGC データが装置でサポートされていない）で止まります（ACS を SBCS で設定したときと同じ）。
  **待ち行列の先頭で詰まると後続もすべて止まります**。
  - 詰まったスプールは**取得経路（pull）で救出**します（書き出しプログラムを通らないので装置の制約を
    受けません）。日本語も読め（SO/SI は ACS と同じく 1 桁ずつ空けて描くので、全角の行は 1 桁右から始まります）、
    救出後のホスト側スプールは `rescueAction` で
...
- **用紙タイプの問い合わせ（MSGW）**: ライターはスプールの用紙タイプに対し `CPA3394`（Load form type '*STD' …）
  ...
  - **DBCS（5553）の装置では、用紙の問い合わせの後に位置合わせの問い合わせ `CPA4044`（印刷装置の位置合わせを
    確認してください）も来ます**（日本語機・PUB400 とも実測。ACS の申告でも同じ）。これにも「I」で応答します。
...
- **失敗したらホストへの印刷完了の応答を止めます**（ACS と同じ）: 自動出力に失敗した帳票は、画面の上部の
  **「再試行」「取消（印刷済みとして応答）」**を選ぶまで、ホストへ印刷完了を返しません。その間スプールはホストに
  残る（書き出し中＝WTR）ので、`SAVE(*NO)` の帳票も失われません。**再試行**は失敗した出力だけをやり直し、できたら応答します。
  **取消**はそのまま応答し、ホストは印刷済みとみなします（帳票はサーバーの一覧には残ります）。
  止めている間ホストは次の帳票を送ってきません。自動出力を OFF にしている間は、止めずにすぐ応答します
  （止めている最中に OFF にすると、その帳票もすぐ応答します）。
  - **再試行はいまの設定で**行います——止めている間に定義の保存先を直してから再試行すれば、直した先へ書きます
  - 止めている帳票は、サービス画面に **⚠ 応答停止中**、MCP の `wait_spool` / `list_spools` に `held` として出ます
    （MCP からは `retry_printer_output` / `cancel_printer_output`）
  - **止めている間に接続が切れても**（停止・切断・サーバーの再起動）、ホストは印刷済みにせず待ち行列に戻します（RDY）。
    繋ぎ直すと送り直されます（PUB400 では書き出しプログラムの用紙の問い合わせに答えた後。実測）
  - ホストが途中で取り消した・保留したジョブ（CLEAR で閉じた帳票）は止めません（ACS も止めません。スプールはホストに残ります）
  - ホスト変換の印刷データは PDF にできないので `PDF —` と出します（失敗ではないので止めません）
  - 自動印刷（`lp` / Windows の印刷）が 120 秒で終わらなければ止めて失敗として扱います
...
- ~~**IGC（DBCS）属性の帳票は書き出し経路（プリンターセッション）を通れない**。仮想装置が SBCS の
  ...
  → **変えられた**: 申告を ACS と同じ組にすると装置が 5553 として作られ、日本語機で IGC 属性の帳票が届いた
  （`20260921-printer-acs-declaration`）。**SBCS の CCSID で開いたセッション**では従来どおり `CPA3303` で止まる
  （→ [プリンターセッション](#プリンターセッションスプール受信) /
  [`docs/HOST-PRINT-TRANSFORM.md`](docs/HOST-PRINT-TRANSFORM.md)）。
- ~~**印刷先（PDF・自動印刷）が失敗しても、印刷完了を返している**（ACS は応答を保留して利用者の再試行・取り消しを待つ）。
  `SAVE(*NO)` のスプールはホストから消える。~~ → ACS と同じく応答を止めて再試行・取消を待つ（`20260921-printer-hold-response`。
  → [プリンターセッション](#プリンターセッションスプール受信)）
