...
- **接続設定はサーバー保存**（端末をまたいで共有・認証オンなら自分の設定のみ）。自動サインオンのパスワードは
  **AES-256-GCM で暗号化**（→ [接続設定の保存](#接続設定の保存サーバー一元管理)）
...
1. **接続**: 接続画面で「サーバー」プロファイル（`profiles.local.json` 由来・読み取り専用）または「保存済み」接続
   （サーバー保存）をクリック。「＋ 新規接続」からホスト直指定で追加でき、**編集 / 削除**、
   **自動サインオン(RFC 4777) の ON/OFF ＋資格情報**、**セッション種別（表示 / プリンター）** を設定可能
   （プリンターは → [プリンターセッション](#プリンターセッションスプール受信)）。カード表示は種別チップ
   （🖥 表示 / 🖨 プリンター）付きで、**カード / 一覧**を切り替えられます。
   保存済み接続は**サーバーに一元管理**され（→ [接続設定の保存](#接続設定の保存サーバー一元管理)）、
   認証オフなら全ユーザー共通、認証オンなら**自分の接続のみ**が見えます。
2. **操作**:
...
- **認証オフ**: サーバー保存の全接続を誰でも一覧・編集・接続できる（ローカル単一運用）。
- **認証オン**: **自分の接続のみ**（admin は全件）。API は `owner` スコープ（`/api/connections`）。
- **運用者管理の共有プロファイル**（`profiles.json`）は従来どおり**読み取り専用**で存続し、PDF 自動蓄積/印刷など
  信頼設定の受け皿になります。ユーザー作成の接続設定にはこれらの信頼フィールドを持たせません
  （任意パス書き込み・コマンド実行の防止。zod strict で拒否）。
