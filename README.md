...
**MCP サーバー**
- stdio ＋ Streamable HTTP、**18 ツール**（接続設定の一覧〜サインオン〜画面取得〜キー送信〜ジョブ情報〜GUI 選択〜プリンター/PDF）
...
| | 実体 | 信頼設定（自動 PDF / 印刷）を置けるか |
|---|---|---|
| **サーバー設定** | `profiles.json`（ファイル管理・`--profiles` で指定） | **置ける（唯一の置き場）** |
| **自分の設定** | `connections.json`（`owner` スコープ） | 置けない（スキーマが拒否） |
...
- **何に接続できるかは `list_connections` で調べられます**（可視範囲は上表に従います）。
...
| `list_connections` | **使える接続設定の一覧**（`open_session` に渡す参照方法つき。可視範囲は認証状態に従う） |
...
| 自動 PDF / 印刷 | ✅ | ❌（スキーマが拒否） | ❌ |
| `passwordEnv`（CI/CD） | ✅ | ❌ | ❌ |
| 認可 | **認証オンでは admin 専用**（一般は参照も利用も不可） | 所有者スコープ | 制限なし |
...
- **`profile` は認証オンでは admin 専用**です。一般ユーザーには一覧が空で返り、`profile` 名を
  名指しした接続・`signon`・出力設定の解決も `FORBIDDEN` になります（一覧から隠すだけにしない）。
  一般ユーザーへ見せたい場合は、別途「公開設定」を追加する方針です（現時点では未実装）。
- 3 つを同時に指定した場合の優先順位は **`connection` → `profile` → `host`** です（WebSocket も同順）。
- **何が使えるかは `list_connections` で調べられます**。`kind` に応じて `ref` を `profile` / `connection` へ
  渡してください。出力設定（`autoPdfDir` 等）や自動サインオンのユーザー名は**返しません**
  （LLM のコンテキストに残さないため。自動サインオンの有無だけを真偽値で示します）。
...
- **自分の設定** = `connections.json`（`owner` スコープ。作成者のみ）
...
- **PDF 自動蓄積 / 自動印刷は「プリンター種別 × サーバー設定 × 編集可」のときだけ**設定できます。
