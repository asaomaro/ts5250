...
SCS（SNA Character String）を等幅の帳票に展開して表示します。ブラウザの接続フォームで
...
- **プリンターセッションは現状 SBCS のみ**。DBCS（日本語・CCSID 1399）の帳票は SO/SI シフトと DBCS
  フォント対応が別途必要で、tn5250 にも参照実装がないため後続対応（実 SCS 採取が前提）。
- **プリンターの自動 PDF（指定フォルダ蓄積）・物理プリンターへの自動印刷は未実装**。現状はブラウザ内の
  等幅表示とテキスト保存・印刷→PDF まで。サーバー側で SCS→PDF を生成して蓄積・印刷する経路は
  実現可能だが後続（ブラウザ単体では無操作でのフォルダ書き込み・物理印刷はできない）。
