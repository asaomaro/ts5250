 ...
 *
 * ここで使う PCML は**実機のコンパイラが吐いたそのもの**——実機で
...
const PCML = `<pcml version="6.0">
   ...
   <program name="PCMLTST" path="/QSYS.LIB/TESTLIB.LIB/PCMLTST.PGM">
...
describe("call", () => {
  it("**記述どおりのパラメータで呼ぶ**（構造体は 1 本にまとまる）", async () => {
    ...
    // path からライブラリを解いている
    expect(target[0]).toBe("TESTLIB/PCMLTST");
    expect(((await res.json()) as { called: string }).called).toBe("TESTLIB/PCMLTST");
  });
  ...
  it("**サービスプログラムは黙って *PGM として呼ばない**", async () => {
    const text = PCML.replace('path="/QSYS.LIB/TESTLIB.LIB/PCMLTST.PGM"', 'entrypoint="SOMEPROC"');
    const res = await post(appWith({}), "call", {
      source: SOURCE,
      text,
      program: "PCMLTST",
