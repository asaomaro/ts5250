...
describe("任意の SQL を受け付ける（spec D1）", () => {
  // 文字列としての SQL は素通しする。**書ける範囲は IBM i の権限が決める**
  // （2026-07-30 に非クエリ文を実行できるようにした。`host-sql.ts` の冒頭）。
...
/**
 * **ルートの境界**。`/api/host/sql` は 2026-07-30 に非クエリ文（DML / DDL）も実行するように
 * なったが（`host-sql-execute.test.ts`）、**取り込み（CSV）は依然として別ルート**である。
 *
 * ここで固定するのは「文面で弾いていない」ことと「入口が混ざっていない」こと。
 * 前者を誤解すると「検証を緩めれば更新できる」と読めてしまい、
 * 後者が崩れると 1 つのルートに別の規律（コミットメント・件数報告）が混ざる。
 */
describe("ルートの境界（取り込みは別ルートのまま）", () => {
  it("**更新系の SQL も文面では弾かない**（弾いていると誤解させない）", async () => {
