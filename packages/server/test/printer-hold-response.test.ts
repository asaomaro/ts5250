...
// PDF を描いて印刷（存在しないプリンターへの `lp`）を 2 回起こすので、単独で 1.3 秒ほどかかる。全量を並列に回すと既定の 5 秒を
// 超えた（2026-09-21 の全量で 1 回。単独では 3 回とも通った）ので、時間を明示する（`zip-writer.test.ts` と同じ扱い）
describe("再試行は失敗した出力だけ・切断後は再試行できない", { timeout: 30_000 }, () => {
