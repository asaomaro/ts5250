...
describe("buildDdmRecord が列ごとの CCSID を使う", () => {
  it("**同じ表の 273 列と 5035 列を別々に符号化する**", async () => {
    const { buildDdmRecord } = await import("../src/ddm/ddm-connection.js");
...
describe("バッチ件数の上限", () => {
  it("LL(2 バイト)に収まる件数までしか詰めない", async () => {
    const { maxBatchSize } = await import("../src/ddm/ddm-connection.js");
  ...
  it("要求値・形式上限・レコード長上限の最小を採る", async () => {
    const { effectiveBatchSizeFor } = await import("../src/ddm/ddm-connection.js");
