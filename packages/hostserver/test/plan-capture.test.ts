...
describe("CALL の結果セットを計画と一緒に返す", () => {
  ...
  function callConn() {
    ...
    const request = vi.fn(async (frame: Frame) => {
      ...
      if (isStatementRequest(frame.reqId)) prepared = sentSql(frame) ?? "";
 ...
 *
 * 実機で `DYNAMIC RESULT SETS 2` の手続きを採ったときの実測:
 *
 * ```
 * QQUCNT=0 計画記録 0 件  CALL TESTLIB.SQLDEMORS2()   ← 受け皿
...
describe("計画を持つ文の組をすべて返す", () => {
  ...
  it("計画記録を持たない組は落とす", () => {
    const groups = pickAllStatements([
      ...
      shell({ QQRID: 1000, QQUCNT: 0, QQ1000: "CALL TESTLIB.P()" }),
      rec({ QQRID: 3000, QQUCNT: 3, QQ1000: "SELECT A FROM T" })
    ]);
