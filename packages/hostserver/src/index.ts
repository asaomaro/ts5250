...
// 読み書きしたいが、画面エミュレーションは要らない」利用者のために `@as400web/core` から
// 切り出したパッケージだからである。
//
// **逆向きの依存（ここから `@as400web/core` を import すること）を作ってはならない。**
// 切り出しの意味が消える。`test/no-core-dependency.test.ts` が src 全体を走査して検査している。
//
// **`export *` は使わない。** 公開面は列挙する（`@as400web/core` の `index.ts` と同じ方針。
