...
describe("isValidObjectName", () => {
  it("通常名は通る", () => {
    expect(isValidObjectName("TESTLIB")).toBe(true);
...
describe("送受信・管理の呼び出し", () => {
  it("send は /api/host/dtaq/send に data/encoding/key を渡す", async () => {
    ...
    await send(SRC, "TESTLIB", "Q", { data: "hi", encoding: "base64", key: "0010" });
    expect(calls[0]?.url).toContain("/api/host/dtaq/send");
    expect(calls[0]?.body).toMatchObject({ library: "TESTLIB", name: "Q", data: "hi", encoding: "base64", key: "0010" });
  });

  it("receive は entry を返す（空は null）", async () => {
    mockFetch(() => ({ body: { entry: null } }));
    expect((await receive(SRC, "TESTLIB", "Q", { wait: 0 })).entry).toBeNull();
  });
  ...
  it("create/clear/attributes を叩く", async () => {
    ...
    await create(SRC, "TESTLIB", "Q", { maxEntryLength: 100, type: "FIFO" });
    await clear(SRC, "TESTLIB", "Q");
    const a = await attributes(SRC, "TESTLIB", "Q");
  ...
  it("エラー応答は DtaqRequestError（日本語文言）で投げる", async () => {
    ...
    await expect(send(SRC, "TESTLIB", "Q", { data: "x" })).rejects.toBeInstanceOf(DtaqRequestError);
    await expect(send(SRC, "TESTLIB", "Q", { data: "x" })).rejects.toThrow(/見つかりません/);
  });
});
...
describe("listEntries（SQL 経由）", () => {
  it("不正なキュー名は SQL を投げる前に弾く", async () => {
    ...
    await expect(listEntries(SRC, "TESTLIB", "A'B")).rejects.toThrow(/キュー名/);
  ...
  it("SQL がエラーなら DtaqRequestError", async () => {
    ...
    await expect(listEntries(SRC, "TESTLIB", "Q")).rejects.toBeInstanceOf(DtaqRequestError);
  });
});
