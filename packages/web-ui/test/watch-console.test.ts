...
describe("WatchPane", () => {
  ...
  it("行を選ぶとその履歴に切り替わる", async () => {
    ...
    await nextTick();
    expect(w.find(".hist").text()).toContain("MARO1/LOGQ");
...
describe("WatchPane: システムごとの絞り込み", () => {
  ...
  it("そのシステムの監視だけを出す", async () => {
    ...
    deliver({ type: "watch-list", watches: [W1, W2] });
    await nextTick();
    expect(w.text()).toContain("MARO1/ORDERQ");
    expect(w.text()).not.toContain("MARO1/LOGQ");
    w.unmount();
  });
  ...
  it("システムを持たない古いタブは全部出す（作り直させない）", async () => {
    ...
    await nextTick();
    expect(w.text()).toContain("MARO1/ORDERQ");
    expect(w.text()).toContain("MARO1/LOGQ");
    w.unmount();
  });
