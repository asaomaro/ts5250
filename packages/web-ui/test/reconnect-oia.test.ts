...
describe("OIA の再接続表示", () => {
  it("再接続中は「再接続中 (n/5)」を出す（「切断」より優先）", () => {
    ...
    expect(w.text()).toContain("再接続中 (2/5)");
    expect(w.text()).not.toContain("切断");
  });

  it("再接続していない切断は従来どおり「切断」", () => {
    const w = oia(state({ connected: false }));
    expect(w.text()).toContain("切断");
  });

  it("**状態の変化は読み上げに載せる**（フォーカスは奪わない）", () => {
    const w = oia(state({ connected: false, reconnect: { attempt: 1, max: 5 } }));
    expect(w.find('[role="status"]').exists()).toBe(true);
    expect(document.activeElement).toBe(document.body); // 誰にも focus() していない
  });
