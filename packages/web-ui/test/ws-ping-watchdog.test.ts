...
class FakeSocket {
   ...
   * **`close` イベントは自動で飛ばさない。**
   *
   * 本物の `WebSocket` は `close()` のあとイベントを飛ばすが、半開きでは
   * **飛ばない／大きく遅れる**ことがある。ここで再現したいのはそちらなので、
   * イベントはテスト側が `fire("close")` で明示的に起こす。
   * つまり下の検査は「代役がイベントを出さない」ことではなく、
   * **イベントが来なくても実装が切断として扱えるか**を見ている。
   */
  close(): void {
    this.closed += 1;
    this.readyState = 3;
