...
describe("useCursor.nextWordStart", () => {
  ...
  /** セル配列から桁アクセサを作る（" "=空白 / ""=全角の後半桁 / 他=文字。ScreenGrid と同じ約束） */
  function at(cells: Cell[][]): (r: number, c: number) => string {
    return (r, c) => {
      const cell = cells[r - 1]?.[c - 1];
      if (!cell) return " ";
      if (cell.kind === "dbcs-tail") return "";
      if (cell.kind === "so" || cell.kind === "si") return " ";
  ...
  it("空白から Ctrl+→ で次の語頭へ（TEXT1  ␣  TEXT2 → T of TEXT2）", () => {
    ...
    expect(nextWordStart(at(cells), { row: 1, col: 7 }, "right", 1, cols)).toEqual({ row: 1, col: 8 });
  });

  it("語中から Ctrl+→ は次の語頭へ（現在の語は飛ばす）", () => {
    const cells = grid("TEXT1  TEXT2");
    // TEXT1 の中（桁3）から右 → TEXT2 の T（桁8）
    expect(nextWordStart(at(cells), { row: 1, col: 3 }, "right", 1, 12)).toEqual({ row: 1, col: 8 });
  });

  it("Ctrl+← は前の語頭へ（語中なら現在語の先頭）", () => {
    const cells = grid("TEXT1  TEXT2");
    // TEXT2 の中（桁10）から左 → TEXT2 の先頭（桁8）
    expect(nextWordStart(at(cells), { row: 1, col: 10 }, "left", 1, 12)).toEqual({ row: 1, col: 8 });
    // TEXT2 の先頭（桁8）から左 → 前の語 TEXT1 の先頭（桁1）
    expect(nextWordStart(at(cells), { row: 1, col: 8 }, "left", 1, 12)).toEqual({ row: 1, col: 1 });
  });

  it("行をまたいで次/前の語頭へ", () => {
    const cells = grid("AAA    ", "   BBB ");
    // 1 行目 AAA の末尾付近（桁3）から右 → 2 行目 BBB の先頭（row2,col4）
    expect(nextWordStart(at(cells), { row: 1, col: 3 }, "right", 2, 7)).toEqual({ row: 2, col: 4 });
  ...
  it("Ctrl+↑/↓ は同じ列位置で空白をスキップし最も近い非空白セルへ", () => {
    ...
    expect(nextWordStart(at(cells), { row: 3, col: 4 }, "up", 3, 7)).toEqual({ row: 1, col: 4 });
    // 1 行 4 列 D から下 → 2 行 4 列 空白を飛ばし 3 行 4 列 X へ
    expect(nextWordStart(at(cells), { row: 1, col: 4 }, "down", 3, 7)).toEqual({ row: 3, col: 4 });
    // 列 1（全行 A/H/O が埋まる）は隣接行へ 1 つずつ
    expect(nextWordStart(at(cells), { row: 2, col: 1 }, "up", 3, 7)).toEqual({ row: 1, col: 1 });
    expect(nextWordStart(at(cells), { row: 2, col: 1 }, "down", 3, 7)).toEqual({ row: 3, col: 1 });
  });

  it("同一列に非空白が無ければ pos を返す（端で停止）", () => {
    // 4 列目は全行空白
    const cells = grid("ABC ", "HIJ ", "OPQ ");
    expect(nextWordStart(at(cells), { row: 3, col: 4 }, "up", 3, 4)).toEqual({ row: 3, col: 4 });
    expect(nextWordStart(at(cells), { row: 1, col: 4 }, "down", 3, 4)).toEqual({ row: 1, col: 4 });
  });
  ...
  it("全角後半（dbcs-tail）は語頭にしない（lead を語頭とする）", () => {
    ...
    expect(nextWordStart(at(cells), { row: 1, col: 1 }, "right", 1, 4)).toEqual({ row: 1, col: 2 });
  });
});
