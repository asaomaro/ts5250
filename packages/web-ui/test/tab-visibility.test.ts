...
describe("畳んだタブグループのタブも「開いている」", () => {
  // **時間の上限を広げる**: App 全体を動的 import するので、変換だけで単独でも 2.9 秒かかる。
  // 全量の並列実行では既定の 5 秒を越えて落ちていた（`20260921-type-ahead` の節目で、変更前の
  // コミットでも 2 回中 1 回 5045ms で落ちることを確かめた）。見たいのは振る舞いで、速さではない
  it("全タブを畳んでもワークスペースに居られ、バッジは全数を出す", { timeout: 30_000 }, async () => {
