...
describe("コマンド入力支援", () => {
  ...
  it("**必須が空なら実行させない**", async () => {
    ...
    await w.find('input[data-kwd="LIB"]').setValue("TESTLIB");
  ...
  it("**組み上がりが見える**", async () => {
    ...
    expect(w.find('[data-testid="preview"]').text()).toBe("CRTLIB LIB(TESTLIB) TEXT(a b)");
  });
  ...
  it("**「確かめる」でサーバーが組んだ文字列に差し替わる**", async () => {
    ...
    fetchMock.mockReturnValueOnce(reply({ command: "CRTLIB LIB(TESTLIB) TEXT('It''s a test')" }));
  ...
  it("**実行すると走った文字列とメッセージを出す**", async () => {
    ...
    fetchMock.mockReturnValueOnce(
      reply({
        ...
        success: true,
        returnCode: 0,
        messages: [{ id: "CPC2102", text: "ライブラリー TESTLIB が作成された。" }]
      })
    );
    await w.findAll("button").find((b) => b.text() === "実行")!.trigger("click");
    await flushPromises();
    // **走った文字列はサーバーの応答から出す**（画面の下書きではなく）
    expect(w.find('[data-testid="ran"]').text()).toBe("CRTLIB LIB(TESTLIB)");
