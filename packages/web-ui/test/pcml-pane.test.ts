...
const PARSED = {
  ...
  programs: [
    {
      name: "PCMLTST",
      path: "/QSYS.LIB/ASAOLIB.LIB/PCMLTST.PGM",
      fields: [
...
async function loaded(handler?: (route: string, body: unknown) => { status?: number; body: unknown }) {
  ...
  await w.find('[data-testid="pcml-path"]').setValue("/home/ASAO/pcmltst.pcml");
...
describe("呼ぶ", () => {
  ...
  it("**結果は項目の隣に出る**", async () => {
    ...
    expect(w.find('[data-testid="pcml-result"]').text()).toContain("ASAOLIB/PCMLTST");
  });
