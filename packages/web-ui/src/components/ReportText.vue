 ...
 *
 * **SO/SI の印を出しても桁は 1 つも動かない。** 印は文字の流れに挟まず**重ねて**描く。桁は復号器が確保している
 * ——ACS と同じく SO/SI は既定で 1 桁ずつ占める（`20260921-scs-sosi-columns`）ので、印はその桁の中に描く。
 * ~~ホストが組んだ桁と食い違う——DBCS の行も他の行と同じ 39 桁目から始まっており、SO/SI は桁を占めない前提で
 * 組まれている~~ は ACS の描き方と違った（`20260728-scs-dbcs-column-align` D1 を破棄）。
 *
 ...
 */
/**
 * 印の位置。**SO/SI が桁を占めるときはその桁の中**（`margin-left:0`・幅は占める桁ぶん）、占めないときは境目に中心を置く
 * （`.so` の既定）。ACS と同じく SO/SI は既定で 1 桁ずつ占める（`ScsDecoder`。`20260921-scs-sosi-columns`）。
 * 古いデータ（`width` の無い印）は占めない側として描く。
 */
...
/* SO/SI の印は本物の { } と見分けが付くよう淡く描く（画面の `.a-shift` と同じ考え方）。
   **重ねて置く**——文字の流れに挟まないので、出しても消しても桁が 1 つも動かない。
   既定は境目に中心（占めない SO/SI）。占めるときは `markStyle` がその桁の中へ寄せる。
   印は本文ではないので選択・コピーにも混ぜない。 */
.so {
  position: absolute;
  top: 0;
  width: 1ch;
