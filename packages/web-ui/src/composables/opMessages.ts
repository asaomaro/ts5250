 ...
 * 応答待ちが長引いたときの通知。
 *
 * **画面は時間で諦めない**（`ws-handler.onKey` の `timeoutMs: "never"`。原典も実機もそう）。
 * ただし黙って待たせ続けると、時間の掛かる処理と本当に固まったのを利用者が区別できない
 * ——以前は 30 秒で「応答がありませんでした」と**嘘をついて**施錠を解いていた。
 * 同じ 30 秒で、**事実だけを言い、抜け道を添える**ものに置き換える。
 */
export const MSG_WAITING_LONG = "ホストの応答を待っています（Attn / SysReq で中断できます）";
