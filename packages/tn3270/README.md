...
TN3270 プロトコルの純 TypeScript 実装。telnet ネゴシエーション（RFC 1576 / RFC 2355）、
3270 データストリーム解釈、画面モデル、EBCDIC⇔Unicode 変換（`@ts5250/ebcdic`）、
トレース/リプレイを提供する。

対応範囲: **基本 TN3270**（RFC 1576）＋ **基本 TN3270E**（RFC 2355 §9）、
SBCS（CCSID 37）＋ **DBCS（930/939・日本語）**、モデル 2〜5（標準 24x80 ＋ 代替 32x80 / 43x80 / 27x132）、
平文 TCP ＋ TLS。
...
**TN3270E は基本（§9）だけ**——`DATA-3270` と `NVT-DATA` しか要求せず、任意機能
（RESPONSES / BIND-IMAGE / SCS-CTL-CODES / SYSREQ）はどれも合意しない。TN3270E を `DO` で
提示してくるホストとは TN3270E で、提示しないホストとは基本 TN3270 で繋ぐ。

**対象外**: プリンター（3287 / LU type 3）、IND$FILE、構造化フィールドの本格対応。

> ⚠ **`@ts5250/server` から使う場合、モデルは 2（24x80）と 5（27x132）だけ**に絞っている
> （3 = 32 行・4 = 43 行は web-ui の画面型に収まらない）。ライブラリとしては 2〜5 を扱える。
