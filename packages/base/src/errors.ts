...
export type ErrorCode =
  ...
  | "READ_ONLY_SESSION"
  /**
   * **他の使い手がセッションを予約している**（HLLAPI の `Reserve`）。自動操作の最中は
   * 人間の入力を締め出す——同じ画面に 2 人が書くと、打ちかけが別の画面へ送られる。
   *
   * `READ_ONLY_SESSION` と分けている理由: あちらは**そのセッションの性質**で、待っても変わらない。
   * こちらは**一時的**で、予約が解ければ通る（`RESOURCE_BUSY` と同じく 409）。
   */
  | "SESSION_RESERVED"
  /**
