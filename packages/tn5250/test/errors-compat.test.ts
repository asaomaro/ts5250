...
import { As400Error as BaseAs400Error } from "@ts5250/base";
import { SqlError } from "@ts5250/hostserver";
 ...
 * `As400Error` の定義は `@ts5250/base` に 1 つだけ在り、`@ts5250/tn5250` はそれを再輸出し、
 * `@ts5250/hostserver` の `SqlError` はそれを継承している——という**3 パッケージに跨る同一性**を
...
describe("As400Error / Tn5250Error", () => {
  ...
  it("`@ts5250/tn5250` が出す As400Error は `@ts5250/base` の実体そのもの", () => {
    // core が再輸出ではなく自前の複製を持ってしまうと、ここが別クラスになる
    expect(As400Error).toBe(BaseAs400Error);
  });

  it("`@ts5250/hostserver` の例外が `@ts5250/base` の instanceof を通る", () => {
