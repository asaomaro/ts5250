...
export const ORDER = {
  ...
  /**
   * **「オーダー」ではなく、表示データの特殊な文字置換。** SC30-3533 / tn5250
   * （lib5250/codes5250.h）のどちらにも定義が無く（0x15〜0x1D の間の空き番地）、
   * この文字置換自体が「オーダー」ではなく表示データの一部であることは
   * `.aidev/works/20260915-acs-protocol-order-audit` research.md F2, F3 で
   * ACS のデコンパイル済みコア（`PS5250.addChar()`）から確認した。独立した
   * 参照実装（tn5250j `tnvt.java`）にはこの置換が無く（research.md F5）、
   * **ACS 固有の実装上の癖**と見られる。
   * 表示データとして書き込む際、通常はコードページ変換（`codepage.sb2uni()`）を
   * 経るところ、バイト値 28（0x1C）だけは固定で文字 `*`（42）に、対の
   * バイト値 30（0x1E、`UNKNOWN_1E`）は文字 `;`（59）に置き換えて表示する。
   * 「オーダー」としての専用 `case` で扱っているのは実装上の都合であり
   * （既存のオーダー switch の枠組みに載せるのが最小の変更だったため。design.md
   * 「設計方針」参照）、5250 プロトコル上の正式なオーダーという意味ではない。
   *
   * 元の発見の経緯（0x1C）: 実機の標準システム画面「スプール・ファイルの表示」
  ...
  /**
   * `UNKNOWN_1C`（0x1C）の対。ACS の `PS5250.addChar()` がバイト値 30 を文字 `;`
   * （セミコロン）へ置換して表示する（`UNKNOWN_1C` の doc コメント、
   * `.aidev/works/20260915-acs-protocol-order-audit` research.md F3 参照）。
   * 実機での目撃例はまだ無く、ACS のソースコードの読解のみに基づく対応
   * （research.md F6）。research.md「実現性/リスク」の通り、WEA（`ORDER.WEA`）と
   * 同様、遭遇すれば同じ WTD 内の以降の全オーダーが失われる欠陥を予防的に
   * 塞ぐ位置づけ。
   */
  UNKNOWN_1E: 0x1e
} as const;
